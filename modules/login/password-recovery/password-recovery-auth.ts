...
export const classifyAuthReturnFailure = ({
  ...
  const normalizedDetails = `${error || ''} ${errorDescription || ''}`.toLowerCase();

  if (
    INTERNAL_AUTH_ERROR_CODES.has(normalizedCode)
    || /database error|error saving new user|internal server|unexpected failure/.test(
      normalizedDetails,
    )
  ) {
    return 'internal';
...
export const getAuthReturnFailureMessage = (
  ...
  const isInstitutionalInvite =
    options.audience === 'institutional' && options.intent === 'invite';
