...
test("convite de Responsável permanece na apresentação e porta públicas", () => {
  assert.deepEqual(
    resolvePasswordSetupPresentation({
      appFlow: false,
      audience: "student",
      intent: "recovery",
      recoverySource: "responsavel",
      recoveryFlow: null,
      initialKind: "invite",
      authorizedKind: "invite",
    }),
...
test("a audiência institucional prevalece sobre query pública conflitante", () => {
  assert.deepEqual(
    resolvePasswordSetupPresentation({
      appFlow: false,
      audience: "institutional",
