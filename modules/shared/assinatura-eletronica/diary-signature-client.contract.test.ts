...
const [serviceSource, contractSource, panelSource, modalSource, requestIdSource] = await Promise.all([
  ...
  readFile(new URL('../../gestor/gestao/tecnicos/detalhes/components/diarios/DiarioElectronicSignaturePanel.tsx', import.meta.url), 'utf8'),
  readFile(new URL('./ElectronicSignatureActionModal.tsx', import.meta.url), 'utf8'),
  readFile(new URL('./electronic-signature-request-id.ts', import.meta.url), 'utf8'),
]);

test('lookup canônico é a única autoridade de restauração do envelope do diário', () => {
  assert.match(serviceSource, /assinatura_eletronica_obter_envelope_diario_atual/);
  assert.match(serviceSource, /value === null \|\| value === undefined \? null : normalizeEnvelopeDetail\(value\)/);
  assert.match(panelSource, /electronicSignatureService\.getCurrentDiaryEnvelope/);
...
test('solicitação e preparo oficial usam contratos distintos e idempotentes', () => {
  assert.match(serviceSource, /assinatura_eletronica_solicitar_envelope_diario/);
  ...
  assert.doesNotMatch(panelSource, /useDiarioPdfDownload|generateDiarioPdf|html2canvas|jsPDF/);
  assert.match(
    panelSource,
...
test('todos os estados terminais exigem ação explícita para uma nova versão', () => {
  for (const status of ['ASSINADO', 'RECUSADO', 'CANCELADO', 'EXPIRADO', 'SUBSTITUIDO']) {
...
test('finalização oficial ocorre somente após sinal canônico e mantém retry após reload', () => {
  ...
  assert.match(modalSource, /detail\?\.envelope\.status === 'FINALIZANDO'/);
  const recoveryStart = modalSource.indexOf('const canRecoverFinalization = Boolean(');
  ...
  assert.match(recovery, /canonicalParticipant\.order === canonicalParticipants\.length/);
  assert.match(recovery, /canonicalParticipants\.every\(\(participant\) => participant\.status === 'ASSINADO'\)/);
  assert.doesNotMatch(recovery, /profile === 'COORDENADOR'|canonicalParticipant\?\.role === 'COORDENADOR'/);
...
test('cliente aceita a lista canônica de 1..6 sem derivar ordem por papel', () => {
  const requestStart = serviceSource.indexOf('const normalizeDiaryEnvelopeRequest = (');
  const requestEnd = serviceSource.indexOf('const normalizeOptionalEnvelopeDetail', requestStart);
  const requestNormalizer = serviceSource.slice(requestStart, requestEnd);
  assert.ok(requestStart >= 0 && requestEnd > requestStart);
  assert.match(requestNormalizer, /normalizeCanonicalDiaryParticipants\(source\.participants\)/);
  assert.match(serviceSource, /signers\.length < 1[\s\S]*?signers\.length > ELECTRONIC_SIGNATURE_STAMP_MAX_SIGNERS/);
...
test('sessionStorage é usado somente pelo módulo de requestIds', () => {
  ...
  assert.match(requestIdSource, /STORAGE_PREFIX = 'universo:assinatura-eletronica:request:v1'/);
  assert.doesNotMatch(requestIdSource, /envelopeStatus|envelopeDetail|participants|artifact/);
});
