...
import { professorFinanceiroReceiptOptions } from './financeiro.queries';
...
} from './professor-financeiro-receipt.pdf';

interface ProfessorFinanceiroReceiptModalProps {
  professorId: string;
  poloId: string;
  paymentId: string;
  onClose: () => void;
...
const ReceiptPreparationOverlay: React.FC<{
  ...
  const modal = (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label="Preparação do recibo de honorários"
    >
          ...
          <div className="mt-7 flex flex-wrap items-center justify-center gap-3">
            ...
            {error && onRetry ? (
              <button
                type="button"
                onClick={onRetry}
                className="inline-flex items-center gap-2 rounded-xl bg-emerald-600 px-4 py-2.5 text-xs font-black uppercase tracking-wide text-white transition-colors hover:bg-emerald-700"
              >
...
const ProfessorFinanceiroReceiptModal: React.FC<ProfessorFinanceiroReceiptModalProps> = ({
  professorId,
  poloId,
  paymentId,
  onClose,
}) => {
  const receiptQuery = useQuery(professorFinanceiroReceiptOptions({
    professorId,
    poloId,
    paymentId,
  }));
  const previewItem = useMemo<ProfessorFinancialReceiptPreviewItem | null>(() => {
  ...
  if (receiptQuery.isPending) {
    return <ReceiptPreparationOverlay onClose={onClose} />;
  }
  if (receiptQuery.isError || !previewItem) {
    return (
      <ReceiptPreparationOverlay
        error={professorFinancialErrorMessage(receiptQuery.error)}
        onClose={onClose}
  ...
  return (
    <CanonicalDocumentPreviewModal
      ...
      fileNamePrefix="recibo-honorarios"
      onClose={onClose}
      isRenderable={(item) => (
      ...
      createPdf={createProfessorFinancialReceiptPdf}
    />
  );
};

export default ProfessorFinanceiroReceiptModal;
