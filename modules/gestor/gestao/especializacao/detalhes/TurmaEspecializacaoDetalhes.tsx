...
import { useTurmaEspecializacaoRealtime } from './hooks/useTurmaEspecializacaoRealtime';

interface TurmaEspecializacaoDetalhesProps {
  turma: Turma;
  onBack: () => void;
}

const TurmaEspecializacaoDetalhes: React.FC<TurmaEspecializacaoDetalhesProps> = ({ turma, onBack }) => {
  const [activeTab, setActiveTab] = useState('resumo');

  useTurmaEspecializacaoRealtime(turma.id);
        ...
        <div className="max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
            <div className="flex items-center gap-4">
              <button 
                onClick={onBack}
                className="p-2 rounded-xl border border-slate-200 text-slate-400 hover:text-rose-600 hover:border-rose-200 transition-colors bg-slate-50"
              >
                <ArrowLeft size={20} />
              </button>
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <span className="bg-rose-100 text-rose-700 px-2 py-0.5 rounded text-[10px] font-black uppercase tracking-widest border border-rose-200">
                    {turma.codigo}
                  </span>
...
export default TurmaEspecializacaoDetalhes;
