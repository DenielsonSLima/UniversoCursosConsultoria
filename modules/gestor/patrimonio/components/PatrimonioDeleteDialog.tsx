import React, { useId, useState } from 'react';
import { createPortal } from 'react-dom';
import { LoaderCircle, Trash2, TriangleAlert, X } from 'lucide-react';
import { usePatrimonioDialog } from '../hooks/usePatrimonioDialog';
import { createPatrimonioRequestId } from '../patrimonio.service';
import type { PatrimonioItem, RemovePatrimonioInput } from '../patrimonio.types';

interface PatrimonioDeleteDialogProps {
  item: PatrimonioItem;
  isPending: boolean;
  errorMessage?: string;
  onClose: () => void;
  onSubmit: (input: RemovePatrimonioInput) => void;
}

export function PatrimonioDeleteDialog({
  item,
  isPending,
  errorMessage,
  onClose,
  onSubmit,
}: PatrimonioDeleteDialogProps) {
  const titleId = useId();
  const descriptionId = useId();
  const [requestId, setRequestId] = useState(createPatrimonioRequestId);
  const [motivo, setMotivo] = useState('');
  const { dialogRef, initialFocusRef } = usePatrimonioDialog(true, onClose, isPending);

  if (typeof document === 'undefined') return null;

  return createPortal(
    <div
      className="fixed inset-0 z-[1100] flex min-h-[100dvh] items-end justify-center bg-[#001a33]/60 p-0 backdrop-blur-sm sm:items-center sm:p-4"
      role="presentation"
      onMouseDown={(event) => {
        if (event.target === event.currentTarget && !isPending) onClose();
      }}
    >
      <div
        ref={dialogRef}
        role="alertdialog"
        aria-modal="true"
        aria-labelledby={titleId}
        aria-describedby={descriptionId}
        aria-busy={isPending}
        tabIndex={-1}
        className="relative w-full max-w-lg rounded-t-[2rem] border border-white/70 bg-white p-6 shadow-2xl sm:rounded-[2rem] sm:p-8"
      >
        <button type="button" onClick={onClose} disabled={isPending} className="absolute right-4 top-4 rounded-xl p-2 text-slate-400 transition-colors hover:bg-slate-100 hover:text-slate-700 disabled:cursor-not-allowed disabled:opacity-50" aria-label="Fechar confirmação de exclusão">
          <X size={19} />
        </button>

        <div className="flex items-start gap-4 pr-10">
          <div className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-rose-50 text-rose-600">
            <TriangleAlert size={28} aria-hidden="true" />
          </div>
          <div>
            <h2 id={titleId} className="text-xl font-black tracking-tight text-[#001a33]">Excluir cadastro patrimonial?</h2>
            <p className="mt-1 text-sm font-bold text-slate-700">{item.descricao}</p>
            <p id={descriptionId} className="mt-2 text-xs font-medium leading-relaxed text-slate-500">
              O item deixará as listagens ativas e a posição patrimonial, mas continuará preservado no histórico de auditoria. Esta ação é permitida somente para um cadastro sem baixa.
            </p>
          </div>
        </div>

        <label className="mt-6 block">
          <span className="mb-1.5 block text-[11px] font-black uppercase tracking-wider text-slate-500">Motivo da exclusão</span>
          <textarea
            required
            maxLength={500}
            rows={3}
            value={motivo}
            onChange={(event) => {
              setMotivo(event.target.value);
              setRequestId(createPatrimonioRequestId());
            }}
            disabled={isPending}
            placeholder="Explique por que este cadastro deve ser excluído"
            className="w-full resize-y rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5 text-sm text-slate-700 outline-none transition-all placeholder:text-slate-400 focus:border-rose-400 focus:bg-white focus:ring-2 focus:ring-rose-100 disabled:cursor-not-allowed disabled:opacity-60"
          />
        </label>

        {errorMessage ? (
          <div role="alert" className="mt-4 rounded-xl border border-rose-100 bg-rose-50 px-4 py-3 text-xs font-semibold leading-relaxed text-rose-700">
            {errorMessage}
          </div>
        ) : null}

        <footer className="mt-7 flex flex-col-reverse gap-3 sm:flex-row">
          <button ref={(element) => { initialFocusRef.current = element; }} type="button" onClick={onClose} disabled={isPending} className="flex-1 rounded-xl border border-slate-200 px-4 py-3 text-xs font-black uppercase tracking-wide text-slate-600 transition-colors hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">
            Cancelar
          </button>
          <button
            type="button"
            onClick={() => {
              if (!motivo.trim()) return;
              onSubmit({
                requestId,
                patrimonioId: item.id,
                poloId: item.poloId,
                expectedUpdatedAt: item.updatedAt,
                motivo: motivo.trim(),
              });
            }}
            disabled={isPending || !motivo.trim()}
            className="inline-flex flex-1 items-center justify-center gap-2 rounded-xl bg-rose-600 px-4 py-3 text-xs font-black uppercase tracking-wide text-white shadow-lg shadow-rose-950/15 transition-colors hover:bg-rose-700 disabled:cursor-not-allowed disabled:opacity-60"
          >
            {isPending ? <LoaderCircle size={15} className="animate-spin" /> : <Trash2 size={15} />}
            {isPending ? 'Excluindo...' : 'Excluir cadastro'}
          </button>
        </footer>
      </div>
    </div>,
    document.body,
  );
}
