...
export type ReportPdfPreviewTone = 'emerald' | 'rose' | 'blue' | 'slate' | 'amber';
...
export type ReportPdfFactory = (
  reportProgress: (message: string) => void,
) => Promise<PreparedReportPdf>;

export interface ReportPdfPreviewModalProps {
  title: string;
  description?: string;
  tone?: ReportPdfPreviewTone;
  preparePdf: ReportPdfFactory;
  onClose: () => void;
}

export const reportPdfPreviewToneStyles: Record<
  ReportPdfPreviewTone,
  { button: string; soft: string; text: string }
> = {
  emerald: {
...
const ReportPdfPreviewModal: React.FC<ReportPdfPreviewModalProps> = ({
  title,
  description,
  tone = 'slate',
  preparePdf,
  ...
  const [preparedPdf, setPreparedPdf] = useState<PreparedReportPdf | null>(null);
  const [previewUrl, setPreviewUrl] = useState<string | null>(null);
  const [isPreparingPdf, setIsPreparingPdf] = useState(true);
  const [progress, setProgress] = useState('Preparando o PDF vetorial...');
  ...
  const isBusy = isPreparingPdf || operation !== null;
  ...
  useEffect(() => {
    ...
    void preparePdf((message) => {
      if (active) setProgress(message);
    })
      .then((result) => {
        assertPdfBlobReady(result.blob, 'O relatório');
        if (!active) return;
      ...
      .catch((failure) => {
        if (!active) return;
        console.error('Não foi possível preparar a prévia do relatório em PDF:', failure);
        setPdfError(
  ...
  const handleDownload = () => {
    ...
    setPdfError(null);
    try {
      assertPdfBlobReady(preparedPdf.blob, 'O relatório');
  ...
  return createPortal(
    <div
      ref={dialogRef}
      tabIndex={-1}
      className="fixed inset-0 z-[140] flex items-center justify-center bg-slate-950/70 p-0 outline-none backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
      ...
      <div className="flex h-full w-full flex-col overflow-hidden bg-white shadow-2xl">
        <header className="flex flex-col gap-3 border-b border-slate-100 px-4 py-3 sm:flex-row sm:items-center sm:justify-between sm:px-5 sm:py-4">
          <div className="flex min-w-0 items-center gap-3">
            <span className={`flex h-10 w-10 shrink-0 items-center justify-center rounded-xl ${toneStyle.soft}`} aria-hidden="true">
              <FileText size={19} />
            </span>
            <div className="min-w-0">
              <h2 id={titleId} className="truncate text-base font-black uppercase tracking-tight text-[#001a33]">
                {title}
              </h2>
              <p
                id={statusId}
                ...
                aria-atomic="true"
              >
                {isPreparingPdf ? progress : description || 'Prévia, download e impressão usam o mesmo PDF.'}
              </p>
            </div>
          </div>
          <div className="flex flex-wrap items-center gap-2 self-end sm:self-auto">
            ...
            <button
              ...
              disabled={!preparedPdf || isBusy}
              aria-label={`Imprimir ${title} em PDF`}
