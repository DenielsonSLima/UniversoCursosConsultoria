...
import { getBlocks } from '../../../cadastros/modelos-documentos/diploma/components/DiplomaPreview';
...
const replaceVars = (text: string, certificado: CertificadoAcademico, extraVars: Record<string, string> = {}) => {
  const vars: Record<string, string> = {
    ...
    data_conclusao: new Date(`${certificado.data_conclusao}T12:00:00`).toLocaleDateString('pt-BR'),
  ...
  return Object.entries(mergedVars).reduce(
    (result, [key, value]) => result.replace(new RegExp(`{{${key}}}`, 'g'), `<strong>${value}</strong>`),
    text || ''
  );
};

const getBackgroundUrl = (modelo: any, page: 'frente' | 'verso') => {
  if (!modelo) return '';
  if (page === 'frente') {
    return modelo.bgFrenteUrl || modelo.frenteUrl || modelo.backgroundFrenteUrl || modelo.bg_frente_url || '';
  }
  return modelo.bgVersoUrl || modelo.versoUrl || modelo.backgroundVersoUrl || modelo.bg_verso_url || '';
};
...
const CertificadoPreview: React.FC<CertificadoPreviewProps> = ({ certificado, modelo, gradeCurricular, pdfMode = false }) => {
  ...
  const renderBlock = (block: any) => {
    switch (block.type) {
      ...
      case 'signature': {
        ...
        return (
          <div style={{ width: block.width || 256 }} className="text-center flex flex-col items-center justify-end">
            <div className="w-full border-t border-slate-400 pt-[1px] mb-2">
              <p
      ...
      case 'table':
        return (
          <div style={{ width: block.width || 560 }} className="rounded-xl border border-slate-200 bg-white/90 p-4">
            ...
            <div className="whitespace-pre-wrap font-mono text-[11px] leading-relaxed text-slate-700" dangerouslySetInnerHTML={{ __html: replaceVars(block.content || '', certificado, templateVars) }} />
          </div>
        );
      case 'registry':
        return (
  ...
  const renderVisualPage = (page: 'frente' | 'verso') => {
    ...
    const style: React.CSSProperties = {
      ...
      backgroundImage: getBackgroundUrl(modelo, page) ? `url(${getBackgroundUrl(modelo, page)})` : undefined,
    ...
    return (
      <section
        ...
        style={style}
      >
        {page === 'verso' && modelo?.landscapeWatermarkUrl && (
        ...
        {blocks.map((block: any) => (
          <div key={block.id} className="absolute z-10" style={{ left: `${block.x}%`, top: `${block.y}%` }}>
            {renderBlock(block)}
          </div>
        ))}
      </section>
