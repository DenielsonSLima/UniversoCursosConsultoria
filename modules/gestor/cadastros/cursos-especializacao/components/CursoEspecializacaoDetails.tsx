...
import { 
  ...
  BookOpen, Layers, Plus, Trash2, CornerDownRight, Award 
} from 'lucide-react';
import { CursoEspecializacao, Modulo, Disciplina, Aula } from '../cursos-especializacao.types';
import { cursosEspecializacaoService } from '../cursos-especializacao.service';

interface CursoEspecializacaoDetailsProps {
  curso: CursoEspecializacao;
  onBack: () => void;
  onUpdate: () => void;
}

const CursoEspecializacaoDetails: React.FC<CursoEspecializacaoDetailsProps> = ({ curso, onBack, onUpdate }) => {
  const [currentCurso, setCurrentCurso] = useState<CursoEspecializacao>(JSON.parse(JSON.stringify(curso)));
  ...
  // --- LOGICA IDENTICA AOS OUTROS MÓDULOS ---
  ...
  const handleSave = async () => {
    setIsSaving(true);
    await cursosEspecializacaoService.update(currentCurso);
    setIsSaving(false);
    onUpdate();
    alert('Especialização salva com sucesso!');
  };
    ...
    <div className="flex flex-col h-full animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-100 pb-6">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 rounded-xl border border-slate-200 text-slate-400 hover:text-rose-600 hover:border-rose-200 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-2 bg-[#001a33] text-white px-6 py-3 rounded-xl font-bold uppercase text-xs tracking-wider hover:bg-rose-600 transition-colors shadow-lg shadow-rose-900/20 disabled:opacity-70">
          <Save size={16} /> {isSaving ? 'Salvando...' : 'Salvar Especialização'}
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-8 pb-20">
          ...
          <div key={modulo.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="bg-rose-50/50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white rounded-lg text-rose-600 border border-slate-100 shadow-sm">
                  <Layers size={18} />
                </div>
                <div>
                  <h4 className="font-bold text-[#001a33]">{modulo.nome}</h4>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                    {modulo.disciplinas.length} Disciplinas • {modulo.disciplinas.reduce((acc, d) => acc + d.cargaHoraria, 0)}h
                  </p>
                </div>
              </div>
              <button onClick={() => handleRemoveModulo(modulo.id)} className="p-2 text-slate-400 hover:text-red-500 transition-colors">
                <Trash2 size={16} />
              </button>
            </div>
            ...
            <div className="p-4 space-y-4">
              {modulo.disciplinas.length === 0 && (
                <div className="text-center py-6 text-slate-400 text-xs uppercase">Nenhuma disciplina neste módulo</div>
              )}
                ...
                <div key={disc.id} className="border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="bg-white p-4">
                    <div className="space-y-2">
                      {disc.aulas?.map((aula) => (
                        <div key={aula.id} className="flex items-center justify-between group pl-4 border-l-2 border-slate-100 hover:border-rose-400 transition-colors py-1">
                    ...
                    <div className="mt-3 pl-4 flex gap-2 items-center">
                        <CornerDownRight size={12} className="text-rose-500" />
                        <input 
                          type="text" 
                          placeholder="Nova aula/conteúdo..."
                          className="flex-1 text-sm bg-slate-50 border-b border-transparent focus:border-rose-500 outline-none px-2 py-1 transition-colors"
                          value={addingAulaToDiscId === disc.id ? newAulaTitulo : ''}
                          onChange={(e) => {
                            setAddingAulaToDiscId(disc.id);
                            setNewAulaTitulo(e.target.value);
                          }}
                          onKeyDown={(e) => { if(e.key === 'Enter') document.getElementById(`horas-esp-${disc.id}`)?.focus(); }}
                        />
                        <input 
                          id={`horas-esp-${disc.id}`}
                          type="number" 
                          placeholder="Hrs"
                          className="w-16 text-sm bg-slate-50 border-b border-transparent focus:border-rose-500 outline-none px-2 py-1 transition-colors text-center"
                        ...
                        <button 
                          ...
                          className="p-1 bg-rose-100 text-rose-600 rounded hover:bg-rose-600 hover:text-white transition-colors"
                ...
                {addingDiscToModId === modulo.id ? (
                  <div className="flex gap-2">
                    <input 
                      ...
                      className="flex-1 px-4 py-2 bg-white border border-rose-200 rounded-xl outline-none focus:ring-2 focus:ring-rose-100 text-sm"
                      value={newDiscName}
                      onChange={(e) => setNewDiscName(e.target.value)}
                      onKeyDown={(e) => { if(e.key === 'Enter') handleAddDisciplina(modulo.id); }}
                    />
                    <button onClick={() => handleAddDisciplina(modulo.id)} className="px-4 py-2 bg-rose-600 text-white rounded-xl text-xs font-bold uppercase hover:bg-rose-700">
                      Adicionar
                    </button>
                  ...
                  <button 
                    ...
                    className="w-full py-2 border border-dashed border-slate-300 rounded-xl text-slate-400 text-xs font-bold uppercase hover:border-rose-400 hover:text-rose-600 hover:bg-rose-50 transition-all flex items-center justify-center gap-2"
                  >
        ...
        <div className="bg-slate-50 rounded-[2rem] border-2 border-dashed border-slate-200 p-6 flex items-center justify-center gap-3">
           <input 
             ...
             className="w-64 px-4 py-2 rounded-xl border border-slate-200 outline-none focus:border-rose-500 text-sm bg-white"
             value={newModuloName}
             onChange={(e) => setNewModuloName(e.target.value)}
           />
           <button onClick={handleAddModulo} className="px-4 py-2 bg-[#001a33] text-white rounded-xl text-xs font-bold uppercase tracking-wider hover:bg-rose-900 transition-colors">
...
export default CursoEspecializacaoDetails;
