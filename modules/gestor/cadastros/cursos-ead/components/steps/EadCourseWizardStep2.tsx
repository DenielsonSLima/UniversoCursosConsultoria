...
import { AlertTriangle, CheckCircle2, CreditCard, Link2 } from "lucide-react";
  ...
  return (
    <div className="space-y-6 animate-fadeIn">
      <div className="flex items-center gap-3 border-b border-slate-100 pb-4">
        ...
        <div>
          <h4 className="font-black text-lg text-[#001a33] uppercase tracking-tight">
            Financeiro e Checkout Asaas
          </h4>
          <p className="text-slate-400 text-xs font-medium mt-0.5">
            Defina valor, formas de recebimento, parcelamento e regra da taxa
            antes de publicar o curso.
          </p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="space-y-1.5">
          ...
          <select
            ...
            onChange={(e) =>
              setFinanceiroTaxaPagaPor(
                e.target.value as "aluno" | "instituicao",
              )}
          >
      ...
      <div className="rounded-3xl border border-emerald-100 bg-emerald-50/70 p-5">
        <h5 className="mb-3 text-sm font-black uppercase tracking-tight text-[#001a33]">
          Taxa da Asaas já no valor cobrado?
        </h5>
        <p className="text-xs font-medium text-slate-600">
          Quando ativado, o backend acrescenta a taxa estimada ao valor cobrado
          para que o líquido da instituição fique próximo ao preço do curso.
        </p>
        <button
          type="button"
          onClick={() =>
      ...
      <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
        <button
          ...
          className={`rounded-2xl border p-4 text-left transition-all disabled:opacity-50 ${
            financeiroParcelado && financeiroCartao
        ...
        >
          ...
          <span className="mt-2 block text-sm font-black">
            {financeiroParcelado && financeiroCartao ? "Ativo" : "Desativado"}
          </span>
        </button>
        ...
        <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
              Repasse do custo de parcelamento
            </p>
            <h5 className="mt-1 text-sm font-black text-[#001a33]">
              Cobrar do aluno o custo estimado do cartão parcelado
            </h5>
            <p className="mt-1 max-w-2xl text-xs font-semibold leading-relaxed text-slate-500">
              Quando ativo, pagamentos escolhidos no cartão usam um valor
              ajustado para a instituição receber próximo ao valor base após a
          ...
          <button
            ...
            disabled={!financeiroCartao || !financeiroParcelado}
            onClick={() =>
              setFinanceiroRepassarCustoParcelamento((prev) => !prev)}
            className={`w-full rounded-2xl border px-5 py-4 text-left transition-all disabled:opacity-50 lg:w-52 ${
              financeiroRepassarCustoParcelamento && financeiroCartao &&
                financeiroParcelado
        ...
        {financeiroRepassarCustoParcelamento && financeiroMultipleMethods && (
          <div className="mt-4 flex items-start gap-3 rounded-2xl border border-amber-200 bg-amber-50 px-4 py-3">
            ...
            <p className="text-xs font-semibold leading-relaxed text-amber-800">
              Com Pix ou boleto junto do cartão, cada aluno escolhe uma forma no
              checkout. O repasse do parcelamento só altera o valor quando a
              escolha for cartão.
            </p>
          </div>
        )}
          ...
          <div className="rounded-2xl border border-white bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
              <div>
                ...
                <strong className="mt-1 block text-base text-[#001a33]">
                  {formatEadMoney(financeiroSimulation.checkout.grossValue)}
                </strong>
              </div>
              <div>
                <span className="block font-bold text-slate-400">
                ...
                <strong className="mt-1 block text-base text-emerald-700">
                  {formatEadMoney(financeiroSimulation.checkout.netValue)}
                </strong>
              </div>
            </div>
            <p className="mt-3 text-[11px] font-semibold leading-relaxed text-slate-500">
              Taxa estimada na configuração:{" "}
              {formatEadMoney(financeiroSimulation.checkout.feeValue)}.
              {financeiroSimulation.checkout.feeValue > 0 && (
                <span>
                  Antecipação: {formatEadMoney(
                    financeiroSimulation.withoutPass.anticipatedEstimate,
                  )}.
                </span>
              )}
            </p>
            ...
            <div className="rounded-2xl border border-emerald-100 bg-white p-4 shadow-sm">
              ...
              <div className="mt-3 grid grid-cols-2 gap-3 text-xs">
                <div>
                  ...
                  <strong className="mt-1 block text-base text-[#001a33]">
                    {formatEadMoney(financeiroSimulation.withPass.customerPays)}
                  </strong>
                </div>
                <div>
                  <span className="block font-bold text-slate-400">
                  ...
                  <strong className="mt-1 block text-base text-emerald-700">
                    {formatEadMoney(
                      financeiroSimulation.withPass.institutionReceives,
                    )}
                  </strong>
                </div>
              </div>
              <p className="mt-3 text-[11px] font-semibold leading-relaxed text-slate-500">
                Taxa estimada:{" "}
                {formatEadMoney(financeiroSimulation.withPass.fee)}. Antecipando
                tudo, estimativa líquida: {formatEadMoney(
                  financeiroSimulation.withPass.anticipatedEstimate,
                )}.
              </p>
            </div>
          )}
        ...
        <div className="mt-4 grid grid-cols-1 gap-3 text-xs md:grid-cols-2">
          <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
            ...
            <strong className="mt-1 block text-sm text-[#001a33]">
              {formatEadMoney(financeiroSimulation.pixOrBoletoNet)}
            </strong>
            <span className="mt-1 block font-semibold text-slate-500">
              Taxa fixa estimada de{" "}
              {formatEadMoney(financeiroSimulation.pixOrBoletoFixedFee)}.
            </span>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
            <span className="block font-black uppercase tracking-widest text-slate-400">
            ...
            <span className="mt-1 block font-semibold text-slate-500">
              A taxa real depende da sua conta Asaas e da antecipação
              contratada.
            </span>
          </div>
          <div className="rounded-2xl border border-slate-200 bg-white px-4 py-3">
            ...
            <span className="mt-1 block font-semibold text-slate-500">
              {financeiroConsiderarTaxaNoCheckout
                ? "o valor cobrado do aluno já inclui a taxa estimada."
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            ...
            <p className="mt-1 text-xs font-semibold leading-relaxed text-emerald-800">
              Ao publicar, o sistema cria a turma única EAD. Na compra, o
              checkout Asaas é gerado para cada aluno usando estas regras
              financeiras e vinculado à matrícula.
            </p>
          </div>
        </div>
      </div>
