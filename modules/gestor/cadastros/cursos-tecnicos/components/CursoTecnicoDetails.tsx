...
import { 
  ArrowLeft, Clock, Calendar, Plus, Trash2, Save, 
  BookOpen, Layers, Check, Video, CornerDownRight 
} from 'lucide-react';
import { CursoTecnico, Modulo, Disciplina, Aula } from '../cursos-tecnicos.types';
import { cursosTecnicosService } from '../cursos-tecnicos.service';

interface CursoTecnicoDetailsProps {
  curso: CursoTecnico;
  onBack: () => void;
  onUpdate: () => void;
}

const CursoTecnicoDetails: React.FC<CursoTecnicoDetailsProps> = ({ curso, onBack, onUpdate }) => {
  const [currentCurso, setCurrentCurso] = useState<CursoTecnico>(JSON.parse(JSON.stringify(curso)));
  const [isSaving, setIsSaving] = useState(false);
  
  // Estados para inputs
  const [newModuloName, setNewModuloName] = useState('');
  
  // Estado para nova disciplina (agora vinculada a um módulo específico para UX)
  const [addingDiscToModId, setAddingDiscToModId] = useState<string | null>(null);
  const [newDiscName, setNewDiscName] = useState('');

  // Estado para nova aula (vinculada a uma disciplina específica)
  ...
  const handleAddDisciplina = (moduloId: string) => {
    ...
    const novaDisciplina: Disciplina = {
      id: Math.random().toString(36).substr(2, 9),
      nome: newDiscName,
      cargaHoraria: 0, // Inicia com 0, soma das aulas
      aulas: []
    };
  ...
  const handleRemoveDisciplina = (moduloId: string, disciplinaId: string) => {
    if(confirm('Remover disciplina e suas aulas?')) {
      setCurrentCurso(prev => ({
        ...prev,
        modulos: prev.modulos.map(m => {
  ...
  const handleAddAula = (moduloId: string, disciplinaId: string) => {
                ...
                return {
                    ...
                    disciplinas: m.disciplinas.map(d => {
                        if (d.id === disciplinaId) {
                            ...
                            // Recalcula carga horária da disciplina
    ...
    setNewAulaTitulo('');
    setNewAulaHoras('');
    // Não fechamos o input para permitir inserção rápida de várias aulas
    // document.getElementById(`input-aula-${disciplinaId}`)?.focus(); 
  };
  ...
  const handleSave = async () => {
    setIsSaving(true);
    await cursosTecnicosService.update(currentCurso);
    setIsSaving(false);
    onUpdate();
    ...
    <div className="flex flex-col h-full animate-fadeIn">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-slate-100 pb-6">
        <div className="flex items-center gap-4">
          <button onClick={onBack} className="p-2 rounded-xl border border-slate-200 text-slate-400 hover:text-blue-600 hover:border-blue-200 transition-colors">
            <ArrowLeft size={20} />
          </button>
          <div>
        ...
        <button onClick={handleSave} disabled={isSaving} className="flex items-center gap-2 bg-[#001a33] text-white px-6 py-3 rounded-xl font-bold uppercase text-xs tracking-wider hover:bg-blue-900 transition-colors shadow-lg shadow-blue-900/20 disabled:opacity-70">
          <Save size={16} /> {isSaving ? 'Salvando...' : 'Salvar Grade'}
        </button>
      </div>
          ...
          <div key={modulo.id} className="bg-white rounded-[2rem] border border-slate-200 shadow-sm overflow-hidden">
            ...
            <div className="bg-slate-50 px-6 py-4 border-b border-slate-100 flex justify-between items-center">
              <div className="flex items-center gap-3">
                <div className="p-2 bg-white rounded-lg text-emerald-600 border border-slate-100 shadow-sm">
                  <Layers size={18} />
                </div>
                <div>
                  <h4 className="font-bold text-[#001a33]">{modulo.nome}</h4>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">
                    {modulo.disciplinas.length} Disciplinas • {modulo.disciplinas.reduce((acc, d) => acc + d.cargaHoraria, 0)}h Totais
                  </p>
                </div>
              </div>
            ...
            {/* Lista de Disciplinas do Módulo */}
            <div className="p-4 space-y-4">
              ...
              {modulo.disciplinas.map((disc) => (
                <div key={disc.id} className="border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow">
                  ...
                  <div className="bg-blue-50/30 px-5 py-3 flex justify-between items-center border-b border-slate-100">
                  ...
                  {/* Lista de Aulas (Conteúdo) */}
                  <div className="bg-white p-4">
                    <div className="space-y-2">
                      {disc.aulas?.map((aula) => (
                        <div key={aula.id} className="flex items-center justify-between group pl-4 border-l-2 border-slate-100 hover:border-blue-400 transition-colors py-1">
                    ...
                    <div className="mt-3 pl-4 flex gap-2 items-center">
                        ...
                        <input 
                          ...
                          onKeyDown={(e) => { if(e.key === 'Enter') document.getElementById(`horas-${disc.id}`)?.focus(); }}
                        />
                        <input 
                          id={`horas-${disc.id}`}
                          type="number" 
              ...
              <div className="mt-4 pt-4 border-t border-slate-100 border-dashed">
                {addingDiscToModId === modulo.id ? (
                  ...
                  >
                    <Plus size={14} /> Nova Disciplina neste Módulo
                  </button>
                )}
              </div>
            </div>
...
export default CursoTecnicoDetails;
