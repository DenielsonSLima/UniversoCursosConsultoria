...
const DiariosPage: React.FC = () => {
  ...
  // Drag and drop mouse handlers (only active for Front Cover fields)
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      if (!draggingField || !dragStartRef.current || !canvasRef.current || activeTab !== 'capa') return;
      ...
      const currentFields = form.capaCampos || DEFAULT_CAPA_CAMPOS;
      ...
      setForm((prev) => {
        const fields = prev.capaCampos || DEFAULT_CAPA_CAMPOS;
        const nextFields = fields.map((f) =>
          f.id === draggingField ? { ...f, x: Number(newX.toFixed(1)), y: Number(newY.toFixed(1)) } : f
        );
        return { ...prev, capaCampos: nextFields };
      });
    };
  ...
  }, [draggingField, form.capaCampos, snapToGrid, activeTab]);
  ...
  const currentField = capaCampos.find((f) => f.id === selectedFieldId);
            ...
            <div className="space-y-6">
              ...
              {(activeTab === 'capa' || activeTab === 'contracapa') && (
                <div className="grid grid-cols-1 xl:grid-cols-[1fr_340px] gap-8 animate-fadeIn">
                    ...
                    <div className="relative">
                      <div className="mb-2 flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
                          {activeTab === 'capa' ? 'Editor da Capa (Clique e arraste os textos)' : 'Visualização do Verso/Contracapa'}
                        </p>
                        {/* Ruler and Snap helpers (Only shown on front cover) */}
                        {activeTab === 'capa' && (
                      ...
                      >
                        ...
                        {/* Grid lines overlay (Ruler - Capa only) */}
                        {activeTab === 'capa' && showGrid && (
                        ...
                        {/* Selected field crosshairs alignment lines (Capa only) */}
                        {activeTab === 'capa' && currentField && showCrosshairs && (
                          <div className="absolute inset-0 pointer-events-none z-0">
                            <div
                          ...
                          form.imprimirValidacaoContracapa && (
                            <div className="absolute inset-[5%_6%_5%_8%] border border-[#071a33]/25 p-5 flex flex-col justify-between rounded-xl text-[#071a33] z-10 overflow-hidden pointer-events-none select-none">
                              ...
                              <div className="flex justify-between items-start border-b border-[#071a33]/15 pb-2 text-left">
                                <div>
                                  <h3 className="text-[10px] font-black uppercase tracking-tight leading-none">Registro de Validação e Assinatura Eletrônica</h3>
                                  <p className="text-[7px] font-bold text-slate-500 uppercase tracking-wider mt-1">{form.cabecalho || 'UNIVERSO CURSOS E CONSULTORIA'}</p>
                                </div>
                                <span className="bg-[#071a33] text-white text-[7px] font-black uppercase tracking-wider px-1.5 py-0.5 rounded leading-none">
                                  Documento Oficial
                                </span>
                              </div>
                              ...
                              {/* Signature Blocks with Central Signatures Overlay */}
                              <div className="grid grid-cols-2 gap-8 text-center border-t border-[#071a33]/10 pt-2 text-[8px]">
                                <div className="flex flex-col items-center justify-end relative h-10">
                                  {form.diretorAssinaturaRole && centralSignatures?.[form.diretorAssinaturaRole] && (
                                    <img
                                      src={centralSignatures[form.diretorAssinaturaRole]}
                                      alt="Assinatura Diretor"
                                      className="absolute -top-7 h-10 object-contain pointer-events-none"
                                ...
                                <div className="flex flex-col items-center justify-end relative h-10">
                                  {form.secretarioAssinaturaRole && centralSignatures?.[form.secretarioAssinaturaRole] && (
                                    <img
                                      src={centralSignatures[form.secretarioAssinaturaRole]}
                  ...
                  {/* Right Column: Properties Panel (Capa) or Configuration Panel (Contracapa) */}
                  <div className="space-y-6">
                    {activeTab === 'capa' ? (
                      <>
                        {/* Fields List (Capa only) */}
                        ...
                        {currentField ? (
                          ...
                          <div className="rounded-2xl border border-dashed border-slate-200 p-8 text-center text-xs text-slate-400 font-medium">
                            Selecione um campo acima ou clique nele na capa para editar as propriedades.
                          </div>
                        )}
                      </>
                    ) : (
                      // CONTRACAPA: Configuration Panel
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() => {
                                      setForm(prev => ({
                                        ...prev,
                                        diretorAssinaturaRole: prev.diretorAssinaturaRole === role.id ? null : role.id
                                      }));
                                    }}
                                ...
                                return (
                                  <button
                                    ...
                                    onClick={() => {
                                      setForm(prev => ({
                                        ...prev,
                                        secretarioAssinaturaRole: prev.secretarioAssinaturaRole === role.id ? null : role.id
                                      }));
                                    }}
                                    className={`py-1 px-1.5 rounded-lg border text-[9px] font-bold transition-all truncate ${
                                      isSelected
