// File: modules/gestor/cadastros/modelos-documentos/estagio/components/EstagioQrConfig.tsx

import React, { useState, useEffect } from 'react';
import { Save, ArrowLeft, QrCode, Plus, X } from 'lucide-react';
import { estagioService } from '../estagio.service';

interface EstagioQrConfigProps {
  onBack: () => void;
}
...
const EstagioQrConfig: React.FC<EstagioQrConfigProps> = ({ onBack }) => {
  ...
  const loadConfig = async () => {
    const config = await estagioService.getQrConfig();
    if (config) {
  ...
  const handleSave = async () => {
    setLoading(true);
    await estagioService.saveQrConfig({ pattern, separator });
    setLoading(false);
    alert('Padrão de validação do estágio salvo com sucesso!');
    onBack();
  };
    ...
    <div className="animate-fadeIn max-w-4xl mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center mb-8 gap-4 border-b border-slate-100 pb-6">
        <div className="flex items-center gap-4 w-full md:w-auto">
            ...
            <div>
                <h3 className="text-2xl font-black text-[#001a33] uppercase tracking-tight">Configuração do Validador de Estágio</h3>
                <p className="text-slate-500 text-sm font-medium">
                    Defina como o código único de validação (QR Code) do termo de estágio será gerado.
                </p>
            </div>
        </div>
        ...
        <button 
            onClick={handleSave}
            disabled={loading}
            className="flex items-center gap-2 bg-[#001a33] text-white px-6 py-3 rounded-xl font-bold uppercase text-xs tracking-wider hover:bg-teal-900 transition-colors shadow-lg shadow-teal-900/20"
        >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                ...
                <div className="grid grid-cols-2 gap-3">
                    {TOKENS.map(token => (
                        <button
                            ...
                            className="flex items-center justify-between p-3 rounded-xl bg-slate-50 border border-slate-200 hover:border-teal-300 hover:bg-teal-50 transition-all group text-left"
                        >
                            <div>
                                <p className="text-xs font-black text-[#001a33] group-hover:text-teal-700">{token.label}</p>
        ...
        <div className="space-y-6">
                ...
                <div className="flex flex-wrap items-center justify-center gap-2">
                    ...
                    {pattern.map((token, idx) => (
                        <div key={idx} className="flex items-center animate-fadeIn">
                            ...
                            <div className="bg-white px-3 py-2 rounded-lg border border-teal-200 text-teal-700 text-xs font-black shadow-sm flex items-center gap-2">
            ...
            {/* Preview Final */}
            <div className="bg-gradient-to-br from-[#001a33] to-teal-900 p-8 rounded-[2rem] text-white shadow-xl flex flex-col items-center text-center relative overflow-hidden">
                <div className="relative z-10 w-full flex flex-col items-center">
                    <div className="bg-white p-2 rounded-xl mb-4">
                        <QrCode className="text-[#001a33]" size={64} />
                    </div>
                    <p className="text-teal-200 text-xs font-bold uppercase tracking-widest mb-2">Simulação do Código</p>
...
export default EstagioQrConfig;
