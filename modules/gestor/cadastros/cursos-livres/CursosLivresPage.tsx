...
import { ArrowLeft, Zap, Plus, Loader2, X, Copy, CheckCircle2, AlertTriangle, Image as ImageIcon, Save, Banknote, Receipt, CreditCard } from 'lucide-react';
...
const CursosLivresPage: React.FC<CursosLivresPageProps> = ({ readOnly = false }) => {
  ...
  const [newCursoDuracaoMeses, setNewCursoDuracaoMeses] = useState(1);
  ...
  const [areasDisponiveis, setAreasDisponiveis] = useState(['Capacitação', 'Tecnologia', 'Gestão', 'Idiomas', 'Outros']);
  ...
  const handleCreateCurso = async (e: React.FormEvent) => {
    ...
    createMutation.mutate({
      ...
      duracaoMeses: Number.isFinite(newCursoDuracaoMeses) ? newCursoDuracaoMeses : 1,
  ...
  const handleToggleStatus = async (curso: Curso, e: React.MouseEvent) => {
    ...
    const confirmMsg = novoStatus === 'inativo' 
      ? 'Deseja INATIVAR este curso livre?' 
      : 'Deseja reativar este curso livre?';
    ...
    return (
      <div className="flex min-w-0 flex-col h-full animate-fadeIn bg-slate-50 min-h-screen overflow-x-hidden">
        <div className="flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 bg-white px-4 py-5 sm:px-6 border-b border-slate-200">
          <div className="flex min-w-0 items-center gap-4">
            <button
              ...
              className="flex-shrink-0 p-2 rounded-xl border border-slate-200 text-slate-400 hover:text-amber-600 hover:border-amber-200 transition-colors bg-white shadow-sm"
              aria-label="Voltar"
            >
              <ArrowLeft size={20} />
            </button>
            <div className="min-w-0">
              <span className="text-[10px] font-black text-amber-600 uppercase tracking-widest bg-amber-50 px-2.5 py-1 rounded-md">
                Curso Livre
              </span>
          ...
          <button
            type="submit"
            form="create-curso-livre-form"
            disabled={isCreatingCurso || isUploading}
            className="flex w-full sm:w-auto items-center justify-center gap-2 bg-[#001a33] hover:bg-amber-600 text-white px-5 py-3 rounded-xl font-bold uppercase text-xs tracking-wider transition-all disabled:opacity-70 shadow-lg shadow-amber-900/20"
          >
        ...
        <div className="bg-white border-b border-slate-200 px-3 py-3 sm:px-6 sm:py-4">
          <div className="mx-auto grid w-full max-w-5xl grid-cols-1">
            <div className="group relative flex min-w-0 flex-col items-center gap-2 px-1">
              <span className="relative z-10 w-8 h-8 rounded-xl flex items-center justify-center font-bold text-sm transition-all border bg-amber-600 border-amber-600 text-white shadow-md shadow-amber-600/20">
                1
              </span>
              <span className="block w-full truncate text-center text-[10px] font-black uppercase tracking-wide text-amber-600">
                Informações Básicas
              </span>
            </div>
          </div>
        ...
        <div className="flex-1 min-w-0 max-w-4xl w-full mx-auto p-4 sm:p-6 md:p-8">
          <form id="create-curso-livre-form" onSubmit={handleCreateCurso} className="bg-white border border-slate-200 rounded-[2.5rem] p-6 md:p-8 shadow-sm space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="md:col-span-2 space-y-1.5">
                ...
                <input required type="text" value={newCursoNome} onChange={(e) => setNewCursoNome(e.target.value)} placeholder="Ex: Excel Avançado para Negócios" className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-100 focus:border-amber-500 outline-none font-semibold text-slate-800 transition-all" />
              </div>
              ...
              <div className="space-y-1.5">
                <div className="flex items-center justify-between gap-3">
                  ...
                  <button type="button" onClick={() => setShowNewAreaInput((prev) => !prev)} className="text-[10px] font-black uppercase tracking-wider text-amber-600 hover:text-amber-700">
                    + Nova área
                  </button>
                </div>
                <div className="flex flex-col gap-2">
                  <select value={newCursoArea} onChange={(e) => setNewCursoArea(e.target.value)} className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-100 focus:border-amber-500 outline-none font-bold text-slate-800 transition-all cursor-pointer">
                    {areasDisponiveis.map(a => <option key={a} value={a}>{a}</option>)}
                  </select>
                  {showNewAreaInput && (
                    <div className="flex gap-2">
                      <input type="text" value={newAreaName} onChange={(e) => setNewAreaName(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && (e.preventDefault(), handleAddArea())} placeholder="Nome da nova área" className="min-w-0 flex-1 px-4 py-2.5 text-xs bg-white border border-amber-200 rounded-xl focus:ring-2 focus:ring-amber-100 outline-none font-bold text-slate-800" />
                      <button type="button" onClick={handleAddArea} className="px-4 py-2.5 rounded-xl bg-amber-600 text-white text-[10px] font-black uppercase tracking-wider hover:bg-amber-700">Adicionar</button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="space-y-1.5">
                ...
                <input required type="number" min="1" value={newCursoCargaHoraria} onChange={(e) => setNewCursoCargaHoraria(parseInt(e.target.value, 10) || 0)} placeholder="Ex: 40" className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-100 focus:border-amber-500 outline-none font-semibold text-slate-800 transition-all" />
              </div>

              <div className="space-y-1.5">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Versão do Curso</label>
                <input required type="text" value={newCursoVersao} onChange={(e) => setNewCursoVersao(e.target.value)} placeholder="Ex: 1.0" className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-100 focus:border-amber-500 outline-none font-semibold text-slate-800 transition-all" />
              </div>

              <div className="space-y-1.5">
                <label className="block text-[10px] font-black text-slate-400 uppercase tracking-widest">Duração (Meses)</label>
                <input required type="number" min="1" value={newCursoDuracaoMeses} onChange={(e) => setNewCursoDuracaoMeses(parseInt(e.target.value, 10) || 0)} placeholder="Ex: 1" className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-100 focus:border-amber-500 outline-none font-semibold text-slate-800 transition-all" />
              </div>
            </div>
            ...
            <div className="space-y-1.5">
              ...
              <textarea value={newCursoDesc} onChange={(e) => setNewCursoDesc(e.target.value)} rows={4} placeholder="Forneça um breve resumo descrevendo os objetivos do curso, público-alvo e diferenciais." className="w-full px-4 py-3 text-sm bg-slate-50 border border-slate-200 rounded-xl focus:bg-white focus:ring-2 focus:ring-amber-100 focus:border-amber-500 outline-none font-semibold text-slate-800 transition-all resize-none" />
            </div>
            ...
            <div className="border border-slate-200 rounded-3xl p-5 bg-slate-50/60 space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {[
                  ...
                  return (
                    <button type="button" key={method.label} onClick={method.onToggle} className={`flex items-center justify-between rounded-2xl border px-4 py-4 text-left transition-all ${method.enabled ? 'border-amber-200 bg-white text-amber-700 shadow-sm' : 'border-slate-200 bg-white/70 text-slate-400'}`}>
                      <span className="flex items-center gap-2 text-xs font-black uppercase tracking-wide">
                        <Icon size={16} />
                        {method.label}
                      </span>
                      <span className={`h-5 w-5 rounded-full border ${method.enabled ? 'border-amber-500 bg-amber-500' : 'border-slate-300 bg-white'}`} />
                    </button>
                  );
                })}
              </div>
            ...
            <div className="border border-slate-200 rounded-3xl p-5 bg-slate-50/60 space-y-4">
              ...
              <div className="border-2 border-dashed border-slate-200 rounded-3xl p-6 text-center bg-white flex flex-col items-center justify-center gap-4">
                {newCursoImagemUrl ? (
                  <>
                    <img src={newCursoImagemUrl} alt="Capa do curso livre" className="max-h-48 rounded-2xl object-cover border border-slate-200 shadow-sm" />
                ...
                <label className="px-5 py-2.5 bg-amber-600 hover:bg-amber-700 text-white text-xs font-bold uppercase tracking-wider rounded-xl cursor-pointer transition-all shadow-md shadow-amber-600/15">
            ...
            <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
              <input type="checkbox" id="newCursoLivrePublicarSitePage" checked={newCursoPublicarSite} onChange={(e) => setNewCursoPublicarSite(e.target.checked)} className="h-4 w-4 cursor-pointer rounded border-slate-300 text-amber-600 focus:ring-amber-500" />
              <label htmlFor="newCursoLivrePublicarSitePage" className="cursor-pointer select-none text-xs font-bold text-slate-700">
    ...
    <div className="animate-fadeIn relative">
      ...
      {loading ? (
        ...
        <div className="text-center py-20 bg-slate-50 rounded-[2rem] border border-dashed border-slate-300">
           ...
           <p className="text-slate-500 font-medium">Nenhum curso livre {statusFilter === 'ativo' ? 'ativo' : 'inativo'} cadastrado.</p>
        </div>
      ) : (
        <div className="space-y-10">
          {Object.entries(groupedCursos).map(([area, list]) => (
            <div key={area} className="animate-fadeIn">
              <h3 className="text-base font-black text-[#001a33] uppercase tracking-widest border-l-4 border-amber-500 pl-3 mb-6 flex items-center gap-2">
                <span>{area}</span>
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 max-w-2xl w-full max-h-[92vh] overflow-y-auto shadow-2xl border border-slate-100 relative">
            ...
            <div className="mb-6 pr-10">
              <h3 className="text-xl font-black text-[#001a33] uppercase tracking-tight">Novo Curso Livre</h3>
              <p className="mt-1 text-xs font-semibold text-slate-500">Cadastro próprio do curso com capa, publicação e dados para o site.</p>
            </div>
            ...
            <form onSubmit={handleCreateCurso} className="space-y-4">
              <div>
                ...
                <input 
                  ...
                  placeholder="Ex: Excel Avançado para Negócios"
                ...
                <div className="flex flex-col sm:flex-row sm:items-center gap-4 rounded-2xl border border-slate-200 bg-slate-50/70 p-4">
                  {newCursoImagemUrl ? (
                    <div className="relative aspect-video w-full sm:w-40 overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm">
                      <img src={newCursoImagemUrl} alt="Capa do curso livre" className="h-full w-full object-cover" />
                      <button
                        type="button"
                  ...
                  <div className="min-w-0 flex-1">
                    <label className="inline-flex cursor-pointer items-center justify-center rounded-xl bg-[#001a33] px-4 py-2.5 text-[10px] font-black uppercase tracking-wider text-white transition-colors hover:bg-amber-600">
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                <input 
                  type="checkbox"
                  id="newCursoLivrePublicarSite"
                  checked={newCursoPublicarSite}
                  onChange={(e) => setNewCursoPublicarSite(e.target.checked)}
                  className="h-4 w-4 cursor-pointer rounded border-slate-300 text-amber-600 focus:ring-amber-500"
                />
                <label htmlFor="newCursoLivrePublicarSite" className="cursor-pointer select-none text-xs font-bold text-slate-700">
                  Publicar no site público
                </label>
              </div>
              ...
              <div className="pt-4 flex gap-3">
                <button 
                  ...
                  className="flex-1 py-3 bg-[#001a33] text-white rounded-xl font-bold uppercase text-xs tracking-wider hover:bg-amber-600 transition-colors shadow-lg disabled:opacity-75"
                >
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4 animate-fadeIn">
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl border border-slate-100 relative">
            ...
            <h3 className="text-xl font-black text-[#001a33] uppercase tracking-tight mb-6 flex items-center gap-2">
              ...
              <span>Duplicar Curso Livre</span>
            </h3>
          ...
          <div className="bg-white rounded-[2.5rem] p-8 max-w-md w-full shadow-2xl border border-slate-100 relative animate-slideUp">
            ...
            <div className="flex gap-3">
              ...
              <button
                ...
                className="flex-1 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl font-bold uppercase text-[10px] tracking-wider transition-all shadow-md"
              >
                Confirmar
              </button>
            </div>
