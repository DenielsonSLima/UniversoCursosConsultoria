...
import BirthdayPushSettingsCard from './BirthdayPushSettingsCard';
...
const PushPhonePreview = ({ draft }: { draft: PushCampaignDraft }) => (
  <div className="relative mx-auto w-full max-w-[310px] overflow-hidden rounded-[34px] border-[7px] border-[#06182a] bg-[linear-gradient(155deg,#0b2e51,#001a33_55%,#103d69)] p-3 shadow-[0_24px_55px_rgba(0,26,51,.25)]">
    ...
    <div className="mt-2 rounded-[22px] bg-white/95 p-4 shadow-xl backdrop-blur">
      ...
      {draft.image ? (
        <img src={draft.image.publicUrl} alt="" aria-hidden="true" className="mt-3 h-32 w-full rounded-2xl object-cover" />
      ) : null}
  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/65 p-4 backdrop-blur-sm" role="dialog" aria-modal="true" aria-labelledby="push-confirm-title">
    <div className="max-h-[92vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] border border-white/20 bg-white shadow-2xl">
      ...
      <div className="grid gap-4 p-6 sm:grid-cols-2">
        ...
        <div className="sm:col-span-2 rounded-2xl border border-blue-100 bg-blue-50/60 p-4">{draft.image ? <img src={draft.image.publicUrl} alt="Prévia da imagem" className="mb-4 max-h-64 w-full rounded-xl object-cover" /> : null}<p className="text-[10px] font-black uppercase tracking-wider text-blue-600">Mensagem</p><p className="mt-2 font-black text-blue-950">{draft.title}</p><p className="mt-1 text-sm leading-6 text-blue-900/75">{draft.body}</p><p className="mt-3 flex items-center gap-2 text-xs font-bold text-blue-700"><CalendarClock size={14} /> {formatDate(draft.scheduledAt)}</p></div>
...
const HistoryCard: React.FC<{ campaign: PushCampaign }> = ({ campaign }) => {
  ...
  return (
    <article className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm transition hover:border-blue-200">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between"><div className="flex min-w-0 gap-3">{campaign.imageUrl ? <img src={campaign.imageUrl} alt="" aria-hidden="true" loading="lazy" className="h-16 w-20 shrink-0 rounded-xl object-cover ring-1 ring-slate-200" /> : null}<div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><span className={`rounded-full px-2.5 py-1 text-[10px] font-black uppercase tracking-wider ${status.style}`}>{status.label}</span><span className="text-[10px] font-black uppercase tracking-wider text-slate-400">{campaign.audienceLabel}</span></div><h3 className="mt-2 truncate text-sm font-black text-[#001a33]">{campaign.title}</h3><p className="mt-1 line-clamp-2 text-xs font-medium leading-5 text-slate-500">{campaign.body}</p></div></div><p className="shrink-0 text-xs font-bold text-slate-500">{formatDate(campaign.scheduledAt || campaign.createdAt)}</p></div>
      <div className="mt-4 grid grid-cols-3 gap-2"><div className="rounded-xl bg-emerald-50 px-3 py-2"><p className="text-lg font-black text-emerald-700">{campaign.sentCount}</p><p className="text-[10px] font-bold text-emerald-700/70">alunos alcançados</p></div><div className="rounded-xl bg-rose-50 px-3 py-2"><p className="text-lg font-black text-rose-700">{campaign.failedCount}</p><p className="text-[10px] font-bold text-rose-700/70">alunos com falha</p></div><div className="rounded-xl bg-slate-100 px-3 py-2"><p className="text-lg font-black text-slate-700">{campaign.skippedCount}</p><p className="text-[10px] font-bold text-slate-500">ignorados</p></div></div>
      {['queued', 'processing', 'partial'].includes(campaign.status) && <div className="mt-4"><div className="mb-1.5 flex justify-between text-[10px] font-black uppercase tracking-wider text-slate-400"><span>Processamento · {campaign.processedCount}/{campaign.recipientCount}</span><span>{campaign.progressPercent}%</span></div><div className="h-1.5 overflow-hidden rounded-full bg-slate-100"><div className="h-full rounded-full bg-blue-600 transition-all" style={{ width: `${campaign.progressPercent}%` }} /></div></div>}
      <p className="mt-3 text-[10px] font-bold text-slate-400">Criada por {campaign.createdByName || 'Gestor'} · {campaign.recipientCount} aluno{campaign.recipientCount === 1 ? '' : 's'} destinatário{campaign.recipientCount === 1 ? '' : 's'} · {campaign.eligibleDevices} dispositivo{campaign.eligibleDevices === 1 ? '' : 's'} elegível{campaign.eligibleDevices === 1 ? '' : 'eis'} na prévia</p>
    </article>
  );
};
