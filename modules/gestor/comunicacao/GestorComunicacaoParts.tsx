...
import {
  ...
  MessageSquare, Paperclip, Plus, Search, Send, Sparkles, Tag, Trash2, X,
} from 'lucide-react';
...
const ACCEPTED_ATTACHMENTS = 'image/jpeg,image/png,image/gif,image/webp,.pdf,.ppt,.pptx,.doc,.docx,.xls,.xlsx';
          ...
          : filteredChats.map((chat) => {
            ...
            return (
              <button key={chat.id} onClick={() => onActiveChat(chat.id)} className={`w-full flex items-center gap-3 p-3 rounded-2xl text-left transition-all border ${selected ? 'bg-blue-50/80 shadow-sm border-blue-100' : 'hover:bg-slate-50 border-transparent'}`}>
                <div className={`w-11 h-11 rounded-full flex items-center justify-center font-bold text-sm text-white shrink-0 shadow-sm ${chat.remetente_tipo === 'Professor' ? 'bg-purple-600' : 'bg-blue-600'}`}>{chat.remetente_nome.slice(0, 2).toUpperCase()}</div>
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-1.5 mt-2"><div className="w-1.5 h-1.5 rounded-full shrink-0" style={{ backgroundColor: category.cor }} /><span className="text-[10px] text-slate-400 font-semibold">Setor: {category.nome}</span><span className="text-[10px] text-slate-300 font-medium">|</span><span className={`text-[10px] font-bold ${chat.remetente_tipo === 'Professor' ? 'text-purple-600 bg-purple-50' : 'text-blue-600 bg-blue-50'} px-2 py-0.5 rounded-full`}>{chat.remetente_tipo}</span></div>
                </div>
              </button>
            );
          })}
...
export const GestorChatPanel: React.FC<GestorChatPanelProps> = ({
  ...
  pendingFile, uploading, onDelete, onFile, onMessage, onSend, onSolve, onTransfer,
}) => {
  ...
  return (
    <div className="flex-1 flex flex-col bg-white">
      <div className="min-h-[72px] border-b border-slate-100 flex justify-between items-center bg-white px-5 z-10">
        <div className="flex items-center gap-3"><div className={`w-11 h-11 rounded-full flex items-center justify-center font-bold text-sm text-white ${chat.remetente_tipo === 'Professor' ? 'bg-purple-600' : 'bg-blue-600'}`}>{chat.remetente_nome.slice(0, 2).toUpperCase()}</div><div><h3 className="font-bold text-sm text-[#001a33] flex items-center gap-2">{chat.remetente_nome}<span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${chat.remetente_tipo === 'Professor' ? 'bg-purple-50 text-purple-600 border border-purple-100' : 'bg-blue-50 text-blue-600 border border-blue-100'}`}>{chat.remetente_tipo}</span></h3><div className="flex items-center gap-2 mt-1"><span className="text-[11px] font-medium text-slate-500 flex items-center gap-1"><Tag size={9} /> Setor: {getGestorCategoryInfo(categories, chat.categoria_id).nome}</span><span className="w-1 h-1 bg-slate-300 rounded-full" /><span className={`text-[11px] font-semibold flex items-center gap-1 ${chat.status === 'pendente' ? 'text-amber-500' : 'text-emerald-500'}`}>{chat.status === 'pendente' ? <Clock size={9} /> : <CheckCircle size={9} />}{chat.status === 'pendente' ? 'Aberto' : 'Resolvido'}</span></div></div></div>
      ...
      <div className="shrink-0 border-t border-slate-100 bg-white">{chat.status === 'solucionada' ? <div className="px-4 py-4"><div className="text-center bg-emerald-50 border border-emerald-100 text-emerald-600 rounded-xl py-3 text-xs font-bold flex items-center justify-center gap-2"><CheckCircle size={12} /> Atendimento finalizado.</div></div> : <div className="p-3 space-y-2">{pendingFile && <div className="flex items-center gap-2 px-3 py-2 bg-blue-50 border border-blue-100 rounded-xl"><FileIcon type={pendingFile.type} /><span className="text-xs font-bold text-slate-700 truncate flex-1">{pendingFile.name}</span><button onClick={() => onFile(null)} className="p-0.5 hover:bg-blue-200 rounded-full transition-colors"><X size={12} className="text-slate-500" /></button></div>}<div className="flex items-center gap-2"><button onClick={() => fileInputRef.current?.click()} className="p-2.5 text-slate-400 hover:text-blue-600 hover:bg-blue-50 rounded-xl transition-colors shrink-0" title="Anexar arquivo"><Paperclip size={18} /></button><input ref={fileInputRef} type="file" accept={ACCEPTED_ATTACHMENTS} className="hidden" onChange={(event) => { onFile(event.target.files?.[0] || null); event.target.value = ''; }} /><div className="flex-1 bg-slate-50 rounded-xl flex items-center px-4 border border-slate-200 focus-within:border-blue-500 focus-within:bg-white transition-all"><input type="text" value={messageText} onChange={(event) => onMessage(event.target.value)} onKeyDown={(event) => event.key === 'Enter' && !event.shiftKey && onSend()} placeholder="Escreva uma mensagem..." className="w-full bg-transparent border-none outline-none text-sm text-slate-700 py-3 font-medium placeholder:text-slate-400" /></div><button onClick={onSend} disabled={(!messageText.trim() && !pendingFile) || uploading} className="p-2.5 bg-[#001a33] text-white rounded-full hover:bg-blue-900 transition-colors shadow-sm disabled:opacity-40 shrink-0">{uploading ? <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin" /> : <Send size={16} />}</button></div></div>}</div>
    </div>
  );
};
