...
const MessageThread: React.FC<MessageThreadProps> = ({ activeConversation, messages, loadingMessages }) => {
  ...
  return (
  <div ref={scrollRef} className="min-h-0 flex-1 overflow-y-auto p-5 custom-scrollbar">
    {!activeConversation ? (
          ...
          return (
            <div key={message.id} className={`flex ${outgoing ? 'justify-end' : 'justify-start'}`}>
              <div className={`max-w-[76%] rounded-2xl px-4 py-3 shadow-sm ${
                ...
                {showText && (
                  <p className={`whitespace-pre-wrap text-sm font-medium leading-relaxed ${media ? 'mt-2' : ''}`}>{message.conteudo}</p>
                )}
