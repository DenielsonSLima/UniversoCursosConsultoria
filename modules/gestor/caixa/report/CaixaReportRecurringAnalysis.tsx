...
const FinancialColumns: React.FC<{ item: CaixaReportRecurringBreakdown }> = ({ item }) => (
  <>
    {VALUE_COLUMNS.map(([, field, tone]) => (
      <strong key={field} className={`text-right text-[7px] ${tone}`}>
...
const ColumnHeader = () => (
  <div className={`grid ${GRID} gap-1 border-b border-slate-200 px-2 py-1.5 text-right text-[5.5px] font-black uppercase tracking-wider text-slate-400`}>
...
export const CaixaReportRecurringAnalysis: React.FC<{
  ...
  <div>
            ...
            {report.analiseRecorrente.modalidades.map((item) => (
              <div key={item.modalidade} className={`grid ${GRID} items-center gap-1 px-2 py-2`}>
                <div className="min-w-0">
                  <p className="truncate text-[8px] font-black text-slate-800">{item.rotulo}</p>
    ...
    <section className="overflow-hidden rounded-xl border border-slate-200 bg-white">
      ...
      <ColumnHeader />
      {rows.length === 0 ? (
        <p className="px-3 py-8 text-center text-[8px] font-semibold text-slate-400">
          Nenhuma turma parcelada com movimento nesta competência.
        </p>
      ) : (
        <div className="divide-y divide-slate-100">
          {rows.map((item) => (
            <div key={item.turmaId} className={`grid ${GRID} items-center gap-1 px-2 py-2`}>
              <div className="min-w-0">
      ...
      {showTotals && (
        <div className={`grid ${GRID} items-center gap-1 border-t border-blue-100 bg-blue-50 px-2 py-2`}>
          <div>
            <p className="text-[8px] font-black uppercase text-blue-900">Total da carteira</p>
            <p className="text-[6px] text-blue-700">Valores canônicos consolidados pelo backend</p>
          </div>
    ...
    <p className="mt-2 text-[6.5px] leading-3 text-slate-400">
