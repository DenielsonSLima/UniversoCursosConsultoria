...
const ParceiroAlunoSecretaria: React.FC<ParceiroAlunoSecretariaProps> = ({ alunoId }) => {
  ...
  if (loadingAluno || loadingEnrollments) return <div className="flex items-center justify-center py-12"><span className="text-xs font-medium text-slate-500">Carregando dados da secretaria...</span></div>;
  return (
    <div className="space-y-5 text-sm font-sans">
      <header className="flex flex-col items-start justify-between gap-4 border-b border-slate-100 pb-4 sm:flex-row sm:items-center"><div><h3 className="text-lg font-semibold tracking-tight text-[#001a33]">Secretaria do Aluno</h3><p className="font-medium text-slate-500">Ações acadêmicas e acompanhamento de solicitações para este cadastro.</p></div><div className="flex flex-wrap gap-2"><button onClick={() => setBulletinOpen(true)} className="flex items-center gap-1.5 rounded-xl border border-slate-200 px-4 py-2 text-xs font-medium text-slate-700 shadow-sm hover:bg-slate-50"><ScrollText size={14} /> Emitir Boletim</button><button onClick={() => setDeclarationOpen(true)} className="flex items-center gap-1.5 rounded-xl bg-[#001a33] px-4 py-2 text-xs font-medium text-white shadow-md hover:bg-blue-900"><FileText size={14} /> Emitir Declaração</button>{technicalIrpfAvailable && irpfPayments.length ? <button onClick={() => setIrpfOpen(true)} className="flex items-center gap-1.5 rounded-xl bg-teal-600 px-4 py-2 text-xs font-medium text-white shadow-md hover:bg-teal-700"><DollarSign size={14} /> Emitir IRPF</button> : <p className="rounded-xl border border-amber-100 bg-amber-50 px-3 py-2 text-xs font-semibold text-amber-700">IRPF indisponível enquanto não houver matrícula técnica ativa e pagamentos confirmados.</p>}</div></header>
      ...
      <TemplateDocumentModal open={irpfOpen} onClose={() => setIrpfOpen(false)} title="Declaração de Rendimentos (IRPF)" documentTitle="Declaração de Anuidade / Rendimentos Escolares" printAreaId="print-area-irpf" code={irpfValidation.data?.code || irpfCode} validationUrl={irpfUrl} template={irpfTemplate} polo={polo} watermark={watermark} replaceVariables={replaceVariables} accent="emerald" printDisabled={!irpfReleased || !irpfPayments.length || irpfReissue.isPending} onPrint={() => void printRegisteredIrpf()} showFooter beforeDocument={<div className="w-[794px] max-w-full rounded-2xl border border-slate-200 bg-white/95 px-4 py-3 shadow-lg print:hidden"><p className="text-xs font-semibold text-slate-500">Ano-calendário</p><select value={selectedIrpfYear} onChange={(event) => setSelectedIrpfYear(Number(event.target.value))} className="mt-1 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2 text-sm font-semibold text-[#001a33]">{irpfYearOptions.map((option) => <option key={option.year} value={option.year}>{option.year}{option.released ? '' : ` - libera em ${option.releaseLabel}`}</option>)}</select><p className="mt-2 text-xs font-semibold text-slate-500">{irpfReleased ? `Pode emitir a declaração referente aos pagamentos de ${selectedIrpfYear}.` : `Disponível a partir de ${irpfReleaseLabel}.`}</p></div>} />
