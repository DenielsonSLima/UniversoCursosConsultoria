  ...
  return (
    <div className="group bg-white rounded-[2rem] border border-slate-100 hover:border-blue-200 hover:shadow-2xl hover:shadow-blue-900/10 transition-all duration-300 flex flex-col justify-between h-full overflow-hidden relative">
      ...
      <div className="px-6 relative -mt-12 flex-1">
        ...
        <div className="mb-6">
          <div className="flex justify-between items-start mb-2">
            <span className={`px-3 py-1 rounded-full text-[10px] font-semibold uppercase tracking-wider ${isPrefeitura ? 'bg-blue-50 text-blue-600' : 'bg-slate-100 text-slate-600'}`}>
              {data.tipo}
            </span>
          </div>

          <h3 className="text-xl font-bold text-[#001a33] mb-2 line-clamp-2 leading-tight group-hover:text-blue-600 transition-colors">
            {data.nome}
          </h3>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100 group-hover:bg-blue-50 group-hover:border-blue-100 transition-colors">
            <div className="flex items-center gap-2 text-slate-400 mb-1 group-hover:text-blue-400">
              ...
              <span className="text-[10px] font-semibold uppercase">Alunos</span>
            </div>
            <p className="text-lg font-bold text-slate-700 group-hover:text-blue-700">{data.alunosCount}</p>
          </div>
          <div className="bg-slate-50 rounded-2xl p-3 border border-slate-100 group-hover:bg-blue-50 group-hover:border-blue-100 transition-colors">
            <div className="flex items-center gap-2 text-slate-400 mb-1 group-hover:text-blue-400">
              <Users size={16} />
              <span className="text-[10px] font-semibold uppercase">Profs.</span>
            </div>
            <p className="text-lg font-bold text-slate-700 group-hover:text-blue-700">{data.professoresCount}</p>
          </div>
        </div>
      </div>
      ...
      <div className="px-6 pb-6 pt-2">
        <button className="w-full flex items-center justify-between p-4 rounded-xl bg-slate-50 text-slate-600 text-xs font-semibold group-hover:bg-[#001a33] group-hover:text-white transition-all duration-300">
          <span>Gerenciar</span>
