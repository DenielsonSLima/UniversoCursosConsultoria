...
const ProfessorCard: React.FC<ProfessorCardProps> = ({ data, onClick }) => {
  ...
  return (
    <div
      onClick={onClick}
      className="bg-white rounded-[24px] border border-slate-200/60 p-6 hover:shadow-[0_8px_30px_rgb(0,0,0,0.06)] hover:border-purple-300/50 transition-all duration-300 group relative cursor-pointer flex flex-col h-full overflow-hidden"
    >
      ...
      <div className="flex justify-between items-start mb-4 relative z-10">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-[14px] bg-purple-50 text-purple-600 flex items-center justify-center overflow-hidden border border-purple-100 shadow-sm shrink-0">
            <img
              src={data.foto || `https://ui-avatars.com/api/?name=${encodeURIComponent(data.nome)}&background=F3E8FF&color=9333EA&bold=true`}
              alt={data.nome}
              className="w-full h-full object-cover"
            />
          </div>
          <div className="flex flex-col min-w-0">
            <h3 className="text-sm font-bold text-slate-800 line-clamp-1 group-hover:text-purple-600 transition-colors" title={data.nome}>
              {data.nome}
            </h3>
            {data.cpf && (
      ...
      <div className="flex flex-wrap gap-1.5 mb-3 relative z-10">
        ...
        <span className="px-2.5 py-1 bg-purple-50 text-purple-600 text-[10px] font-bold uppercase tracking-widest rounded-full border border-purple-100">
          Professor
        </span>
        {data.titulacao && (
          <span className="flex items-center gap-1 px-2.5 py-1 bg-indigo-50 text-indigo-600 text-[10px] font-bold uppercase tracking-widest rounded-full border border-indigo-100">
            <GraduationCap size={9} />{data.titulacao}
          </span>
        )}
      </div>
      ...
      <div className="flex-1 space-y-2 relative z-10">
        {data.especialidade && (
          <div className="flex items-center gap-2 text-xs text-slate-600">
            ...
            <span className="truncate font-medium">{data.especialidade}</span>
          </div>
        )}
        {data.telefone && (
          <div className="flex items-center gap-2 text-xs text-slate-600">
      ...
      <div className="mt-4 pt-3 border-t border-slate-100 flex items-center justify-between relative z-10">
        <span className="text-[10px] text-slate-400 font-medium">{data.tipoVinculo || data.poloNome || 'Professor'}</span>
        <div className="flex items-center gap-1 text-[11px] font-bold text-slate-400 uppercase tracking-wider group-hover:text-purple-600 transition-colors">
