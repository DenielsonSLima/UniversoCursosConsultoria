...
import { MoreVertical, GraduationCap, MapPin, Mail, Phone, Users, FileText, DollarSign, ChevronRight } from 'lucide-react';
...
const AlunoCard: React.FC<AlunoCardProps> = ({ data, onClick }) => {
  return (
    <div 
        onClick={onClick}
        className="bg-white rounded-[24px] border border-slate-200/60 p-6 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-blue-300/60 transition-all duration-300 group relative cursor-pointer flex flex-col h-full overflow-hidden"
    >
      {/* Decorative gradient blur */}
      <div className="absolute -right-6 -top-6 w-24 h-24 bg-blue-50 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>

      {/* Header Profile */}
      <div className="flex justify-between items-start mb-5 relative z-10">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-[14px] bg-blue-50/50 text-blue-600 flex items-center justify-center overflow-hidden border border-blue-100/50 shadow-sm shrink-0">
             <img 
              src={data.foto || `https://ui-avatars.com/api/?name=${data.nome}&background=E0F2FE&color=2563EB&bold=true`} 
              alt={data.nome}
              className="w-full h-full object-cover" 
            />
          </div>
          <div className="flex flex-col min-w-0">
            <h3 className="text-sm font-bold text-slate-800 line-clamp-1 group-hover:text-blue-600 transition-colors mb-0.5" title={data.nome}>
              {data.nome}
            </h3>
            {data.cpf && (
            ...
            {data.nomeSocial && (
                <div className="text-[11px] font-medium text-slate-500 mt-1.5 flex items-center gap-1.5 bg-blue-50/50 w-fit px-2 py-0.5 rounded-md border border-blue-100/50">
                    <span className="text-blue-400">Social:</span>
                    <span className="text-blue-700 font-bold truncate max-w-[120px]" title={data.nomeSocial}>{data.nomeSocial}</span>
                </div>
            )}
          </div>
        </div>
            ...
            <div className="flex flex-col flex-1 min-w-0 space-y-1">
              {data.nomeMae && (
                <div className="flex items-baseline gap-1.5 truncate">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0">Mãe</span> 
                  <span className="truncate font-medium" title={data.nomeMae}>{data.nomeMae}</span>
                </div>
              )}
              {data.nomePai && (
                <div className="flex items-baseline gap-1.5 truncate">
                  <span className="text-[10px] font-bold text-slate-400 uppercase tracking-wider shrink-0">Pai</span> 
                  <span className="truncate font-medium" title={data.nomePai}>{data.nomePai}</span>
                </div>
              )}
            </div>
          </div>
      ...
      <div className="mt-5 pt-4 border-t border-slate-100 flex items-center justify-between relative z-10">
        ...
        <div className="flex items-center gap-1.5 text-[11px] font-bold text-slate-400 uppercase tracking-wider group-hover:text-blue-600 transition-colors">
