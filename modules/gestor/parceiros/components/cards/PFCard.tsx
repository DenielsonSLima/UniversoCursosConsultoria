...
import { MoreVertical, User, MapPin, Mail, Phone, FileText, DollarSign, ChevronRight } from 'lucide-react';
...
const PFCard: React.FC<PFCardProps> = ({ data, onClick }) => {
  return (
    <div 
        onClick={onClick} 
        className="bg-white rounded-[24px] border border-slate-200/60 p-6 hover:shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:border-amber-300/60 transition-all duration-300 group relative cursor-pointer flex flex-col h-full overflow-hidden"
    >
      {/* Decorative gradient blur */}
      <div className="absolute -right-6 -top-6 w-24 h-24 bg-amber-50 rounded-full blur-2xl opacity-0 group-hover:opacity-100 transition-opacity duration-500 pointer-events-none"></div>

      {/* Header Profile */}
      <div className="flex justify-between items-start mb-5 relative z-10">
        <div className="flex items-center gap-3.5">
          <div className="w-11 h-11 rounded-[14px] bg-amber-50/50 text-amber-600 flex items-center justify-center overflow-hidden border border-amber-100/50 shadow-sm shrink-0">
             <img 
              src={data.foto || `https://ui-avatars.com/api/?name=${data.nome}&background=FEF3C7&color=D97706&bold=true`} 
              alt={data.nome}
              className="w-full h-full object-cover" 
            />
          </div>
          <div className="flex flex-col min-w-0">
            <h3 className="text-sm font-bold text-slate-800 line-clamp-1 group-hover:text-amber-600 transition-colors mb-0.5" title={data.nome}>
              {data.nome}
            </h3>
            {data.cpf && (
            ...
            {data.nomeSocial && (
                <div className="text-[11px] font-medium text-slate-500 mt-1.5 flex items-center gap-1.5 bg-amber-50/50 w-fit px-2 py-0.5 rounded-md border border-amber-100/50">
                    <span className="text-amber-500">Social:</span>
                    <span className="text-amber-700 font-bold truncate max-w-[120px]" title={data.nomeSocial}>{data.nomeSocial}</span>
                </div>
            )}
          </div>
        </div>
      ...
      <div className="flex-1 space-y-3 relative z-10">
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          <span className="px-2.5 py-1 bg-amber-50/50 text-amber-600 text-[10px] font-bold uppercase tracking-widest rounded-md border border-amber-100/50">
             Pessoa Física
          </span>
          
          {data.roles?.map((role: string) => (
             <span key={role} className="px-2.5 py-1 bg-slate-100 text-slate-600 text-[10px] font-bold uppercase tracking-widest rounded-md border border-slate-200">
               {role}
             </span>
          ))}
        </div>
