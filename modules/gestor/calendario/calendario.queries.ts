...
export const gestorCalendarQueryOptions = (poloId?: string | null) => queryOptions({
  ...
  queryFn: async ({ signal }): Promise<GestorCalendarData> => {
    ...
    const teacherById = new Map(
      (dbTeachers || []).map(teacher => [teacher.id, teacher.nome]),
    );
    const turmaById = new Map(
      dbTurmas.map(turma => [turma.id, turma]),
    );
    const enrichedEvents = eventsData.map(event => ({
      ...event,
      professorName: event.professorId
        ? teacherById.get(event.professorId) || event.professorName
        : event.professorName,
      turmaName: event.turmaId
        ? turmaById.get(event.turmaId)?.nome || event.turmaName
        : event.turmaName,
    }));
    ...
    return {
      events: [...enrichedEvents, ...classEvents],
