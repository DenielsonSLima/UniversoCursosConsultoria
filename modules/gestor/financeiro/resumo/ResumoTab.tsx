...
const ResumoTab: React.FC<ResumoTabProps> = ({
  ...
  const initialMonthRange = useMemo(() => getResumoPresetRange('CURRENT_MONTH'), []);
  const [periodPreset, setPeriodPreset] = useState<ResumoPeriodPreset>('CURRENT_MONTH');
  const [customDraft, setCustomDraft] = useState<ResumoPeriodRange>(initialMonthRange);
  const [customRange, setCustomRange] = useState<ResumoPeriodRange>(initialMonthRange);
  useFinanceiroRealtime(poloId);

  const presetRange = periodPreset === 'TODAY'
    ? getResumoPresetRange('TODAY')
    : getResumoPresetRange('CURRENT_MONTH');
  const activeRange = periodPreset === 'CUSTOM' ? customRange : presetRange;
  const overdueRange = getResumoOverdueRange();
  const flowPeriods = getResumoThreeMonthPeriods();
  const customRangeError = validateResumoCustomRange(customDraft);
  ...
  return (
    <div className="space-y-5 animate-fadeIn">
        ...
        <div className="flex flex-col gap-5 xl:flex-row xl:items-start xl:justify-between">
          <div>
            ...
            <p className="mt-1 max-w-2xl text-xs font-semibold leading-relaxed text-slate-500">
              Posição do período, valores vencidos e fluxo realizado para orientar as próximas ações.
            </p>
          </div>

          <div className="min-w-0 xl:max-w-2xl">
            <div className="flex flex-wrap gap-2" aria-label="Período do resumo financeiro">
              {[
                { id: 'TODAY' as const, label: 'Hoje' },
                { id: 'CURRENT_MONTH' as const, label: 'Este mês' },
                { id: 'CUSTOM' as const, label: 'Personalizado' },
              ].map((option) => (
                <button
                  key={option.id}
                  type="button"
                  aria-pressed={periodPreset === option.id}
                  onClick={() => setPeriodPreset(option.id)}
                  className={`min-h-11 rounded-xl border px-4 text-xs font-black uppercase tracking-wide transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500 ${
            ...
            {periodPreset === 'CUSTOM' ? (
              <div className="mt-3 rounded-2xl border border-slate-200 bg-slate-50 p-3">
                <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
                  <label className="flex-1 text-[10px] font-black uppercase tracking-wide text-slate-500">
                    Data inicial
                    <input
                      type="date"
                      value={customDraft.start}
                      onChange={(event) => setCustomDraft((current) => ({ ...current, start: event.target.value }))}
                      className="mt-1 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </label>
                  <label className="flex-1 text-[10px] font-black uppercase tracking-wide text-slate-500">
                    Data final
                    <input
                      ...
                      onChange={(event) => setCustomDraft((current) => ({ ...current, end: event.target.value }))}
                      className="mt-1 min-h-11 w-full rounded-xl border border-slate-200 bg-white px-3 text-xs font-semibold text-slate-700 outline-none focus:ring-2 focus:ring-blue-500"
                    />
                  </label>
                  <button
                    ...
                    className="min-h-11 rounded-xl bg-[#001a33] px-5 text-xs font-black uppercase tracking-wide text-white transition-colors hover:bg-slate-800 disabled:cursor-not-allowed disabled:opacity-40 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                  >
                    Aplicar
                  </button>
                </div>
                {customRangeError ? <p className="mt-2 text-xs font-semibold text-rose-600" role="alert">{customRangeError}</p> : null}
              </div>
            ) : null}
          </div>
        </div>

        <div className="mt-4 flex flex-wrap items-center gap-2 border-t border-slate-100 pt-4 text-[10px] font-bold text-slate-500">
          <CalendarDays size={14} className="text-blue-600" />
          Período aplicado: <span className="text-slate-700">{formatResumoRange(activeRange)}</span>
          {summaryQuery.isFetching && !summaryQuery.isLoading ? (
            <span className="ml-auto inline-flex items-center gap-1 text-blue-600"><Loader2 size={12} className="animate-spin" /> Atualizando</span>
          ) : null}
        </div>
      </section>
      ...
      <section aria-labelledby="resumo-posicao-title">
        <div className="mb-3">
          <h3 id="resumo-posicao-title" className="text-lg font-black tracking-tight text-[#001a33]">Posição financeira</h3>
          <p className="text-xs font-medium text-slate-500">Saldo atual e movimentação no período selecionado.</p>
        </div>
        {summaryQuery.isLoading ? (
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
            ...
            <KpiCard label="Recebido" value={formatCurrency(summaryQuery.data.totalRecebido)} detail="Pagamento no período" icon={ArrowDownLeft} iconClassName="bg-emerald-50 text-emerald-600" />
            <KpiCard label="Pago" value={formatCurrency(summaryQuery.data.totalPago)} detail="Saída paga no período" icon={ArrowUpRight} iconClassName="bg-rose-50 text-rose-600" />
            <KpiCard label="A receber" value={formatCurrency(summaryQuery.data.totalAReceber)} detail="Vencimento no período" icon={Clock3} iconClassName="bg-amber-50 text-amber-600" />
            <KpiCard label="A pagar" value={formatCurrency(summaryQuery.data.totalAPagar)} detail="Vencimento no período" icon={CalendarDays} iconClassName="bg-indigo-50 text-indigo-600" />
          </div>
        ) : null}
      </section>
      ...
      <section className="rounded-3xl border border-slate-100 bg-slate-50/70 p-5" aria-labelledby="resumo-atencao-title">
        ...
        <div className="mt-4">
          {overdueQuery.isLoading ? (
            <div className="grid gap-3 md:grid-cols-2"><CardSkeleton /><CardSkeleton /></div>
                  ...
                  <div className="mt-2 flex flex-wrap items-center justify-between gap-2">
                    ...
                    {item.enabled ? (
                      <button
                        ...
                        className="inline-flex min-h-11 items-center gap-1 rounded-xl bg-white/80 px-3 text-[10px] font-black uppercase tracking-wide shadow-sm transition-colors hover:bg-white focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-current"
                      >
      ...
      <section className="rounded-3xl border border-slate-100 bg-white p-5 shadow-sm" aria-labelledby="resumo-fluxo-title">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs font-medium text-slate-500">Recebimentos brutos e pagamentos efetivados em cada mês, sem projeções.</p>
          </div>
        ...
        <div className="mt-4">
          {flowQuery.isLoading ? (
            <div className="grid gap-3 md:grid-cols-3"><CardSkeleton /><CardSkeleton /><CardSkeleton /></div>
          ) : flowQuery.isError ? (
      ...
      {quickActions.length > 0 ? (
        <section aria-labelledby="resumo-atalhos-title">
            ...
            {quickActions.map((action) => {
              ...
              return (
                <button
                  ...
                  className="flex min-h-14 items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-4 text-left text-xs font-black text-slate-700 shadow-sm transition-all hover:border-blue-200 hover:text-blue-700 hover:shadow-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-blue-500"
                >
