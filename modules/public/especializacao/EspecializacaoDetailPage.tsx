...
const EspecializacaoDetailPage: React.FC = () => {
  ...
  const { data: curso, isLoading: loadingCurso, error: errorCurso } = useQuery<Curso>({
    queryKey: ['especializacaoPublicDetail', id],
  ...
  const { data: modulos = [], isLoading: loadingGrade } = useQuery<Modulo[]>({
    queryKey: ['especializacaoPublicGrade', id],
  ...
  const { data: turmasAbertas = [] } = useQuery<any[]>({
    queryKey: ['especializacaoPublicTurmasAbertas', id],
        ...
        <main className="flex-grow flex items-center justify-center py-20 px-6 bg-slate-50">
          <div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-3xl p-8 md:p-12 shadow-2xl text-center">
            ...
            <p className="text-slate-300 text-sm leading-relaxed mb-8 font-light">
              Estamos implementando as subpáginas detalhadas para cada uma das nossas especializações. Em breve você poderá conferir o conteúdo programático de cada curso!
            </p>
            <button
              onClick={() => navigate('/especializacao')}
    ...
    return (
      <div className="flex flex-col min-h-screen bg-slate-50 font-sans">
        ...
        <div className="flex-grow flex flex-col items-center justify-center py-40 gap-4">
          ...
          <p className="text-slate-500 font-bold uppercase tracking-wider text-xs">Carregando detalhes da especialização...</p>
        </div>
        <Footer />
      </div>
    );
    ...
    return (
      <div className="flex flex-col min-h-screen bg-slate-50 font-sans">
        ...
        <div className="flex-grow flex flex-col items-center justify-center py-32 px-6 text-center max-w-md mx-auto">
          ...
          <h2 className="text-2xl font-black text-slate-800 uppercase tracking-tight mb-2">Especialização Não Encontrada</h2>
          <p className="text-slate-500 text-sm leading-relaxed mb-8">
            O curso solicitado não foi encontrado ou não está disponível para exibição pública no momento.
          </p>
          <button
            onClick={() => navigate('/especializacao')}
          ...
          >
            Ver Catálogo de Especializações
          </button>
        </div>
        <Footer />
      </div>
            ...
            <div className="lg:col-span-7 space-y-6">
              ...
              <div className="flex flex-wrap gap-4 pt-2">
                ...
                <div className="flex items-center gap-2 bg-white/5 border border-white/10 px-4 py-2.5 rounded-2xl">
                  ...
                  <div>
                    <p className="text-[9px] font-bold text-slate-400 uppercase tracking-wider">Especialidade</p>
            ...
            <div className="lg:col-span-7 space-y-12">
              ...
              <div className="bg-white rounded-[2rem] border border-slate-100 p-8 shadow-sm space-y-4">
                ...
                <p className="text-slate-600 text-sm leading-relaxed whitespace-pre-line font-medium">
                  {curso.descricao || 'Este curso de especialização técnica oferece formação de pós-técnico qualificada para profissionais que já possuem habilitação técnica prévia e buscam um direcionamento ou aprofundamento específico em sua área de atuação.'}
                </p>
              </div>

              {/* Pré-requisito Obrigatório */}
              <div className="bg-amber-50/50 border border-amber-200/60 rounded-[2rem] p-6 shadow-sm flex gap-4 items-start">
                <div className="w-10 h-10 rounded-2xl bg-amber-100/80 text-amber-700 flex items-center justify-center shrink-0 border border-amber-200">
                  <Award size={20} />
                </div>
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-amber-900 leading-relaxed font-medium">
                    {curso.nome.toLowerCase().includes('tdah')
                      ? 'Para efetivar a matrícula neste curso, é necessário comprovar formação técnica ou graduação concluída na área de Educação ou Saúde (como Pedagogia, Psicologia, etc.).'
                      : curso.nome.toLowerCase().includes('instrumentação') || curso.nome.toLowerCase().includes('enfermagem')
                      ? 'Para efetivar a matrícula neste curso, é obrigatório possuir diploma ou declaração de conclusão do curso Técnico em Enfermagem (com registro no COREN ativo).'
                      : 'Para realizar este curso de especialização, é obrigatório possuir habilitação técnica ou graduação correlata.'}
                  </p>
                </div>
              </div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4">
                        {curso.imagem_detalhe_1 && (
                          <div className="bg-white border border-slate-100 p-3 rounded-[2rem] shadow-sm overflow-hidden aspect-[16/10]">
                            <img src={curso.imagem_detalhe_1} alt="Ambiente prático" className="w-full h-full object-cover rounded-2xl" />
                          </div>
                        )}
              ...
              <div className="bg-gradient-to-br from-[#001a33] to-[#003366] rounded-[2.5rem] p-8 text-white relative overflow-hidden border border-blue-950/20 shadow-xl">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  <div className="flex gap-3">
                    ...
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-blue-300">Foco Profissional</h4>
                      <p className="text-[11px] text-slate-300 font-light mt-1 leading-normal">Formação direcionada para atuação imediata em nichos de alta demanda.</p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <CheckCircle2 className="text-blue-400 shrink-0 mt-0.5" size={18} />
                  ...
                  <div className="flex gap-3">
                    <CheckCircle2 className="text-blue-400 shrink-0 mt-0.5" size={18} />
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-blue-300">Docentes Atuantes</h4>
                      <p className="text-[11px] text-slate-300 font-light mt-1 leading-normal">Aulas ministradas por especialistas com ampla experiência prática no mercado.</p>
                    </div>
                  </div>
                  <div className="flex gap-3">
                    <CheckCircle2 className="text-blue-400 shrink-0 mt-0.5" size={18} />
                    <div>
                      <h4 className="text-xs font-bold uppercase tracking-wider text-blue-300">Certificação Válida</h4>
                      <p className="text-[11px] text-slate-300 font-light mt-1 leading-normal">Certificado reconhecido nacionalmente com validade para concursos e conselhos de classe.</p>
                    </div>
                  </div>
                </div>
              </div>
...
export default EspecializacaoDetailPage;
