...
Deno.test('contrato e carteirinha de preceptor mantêm uma allowlist pública mínima', () => {
  const fieldsFor = (documentType: 'contrato_aluno' | 'carteirinha_preceptor') => (
    PUBLIC_VALIDATION_FIELD_CATALOG
      .filter((field) => isPublicValidationFieldCompatible(field, documentType))
      .map((field) => field.id)
  );

  assert.deepEqual(fieldsFor('contrato_aluno'), [
    'studentName',
    'courseName',
    'className',
    'institutionName',
  ...
  assert.deepEqual(fieldsFor('carteirinha_preceptor'), [
    'studentName',
    'institutionName',
    'institutionCnpj',
    'unitName',
  ...
  assert.equal(
    isPublicValidationFieldAllowedForDocument('studentCpf', 'contrato_aluno'),
    false,
  );
  assert.equal(
    isPublicValidationFieldAllowedForDocument('studentPhotoUrl', 'carteirinha_preceptor'),
    false,
  );
});
