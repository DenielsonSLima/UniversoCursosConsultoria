...
const AlunoMobileHome = ({
  ...
  return (
    <div className="space-y-4 md:hidden">
      {hasDataError ? (
        <div role="alert" className="flex items-center gap-3 rounded-2xl border border-amber-200 bg-amber-50 p-3 text-amber-900 shadow-sm">
          <AlertTriangle size={19} className="shrink-0 text-amber-600" />
          <p className="min-w-0 flex-1 text-xs font-bold leading-relaxed">Algumas informações não foram carregadas.</p>
          <button type="button" onClick={onRetry} className="flex h-11 shrink-0 items-center gap-2 rounded-xl bg-white px-3 text-[10px] font-black uppercase tracking-wide text-amber-800 shadow-sm" aria-label="Tentar carregar novamente">
            <RefreshCw size={15} /> Tentar
          </button>
        </div>
      ) : null}
      <section className="relative overflow-hidden rounded-[1.75rem] bg-[#001a33] px-5 pb-5 pt-5 text-white shadow-[0_18px_45px_rgba(0,26,51,0.18)]">
        ...
        {primaryCourse ? (
          ...
          <div className="relative mt-5 rounded-2xl border border-amber-300/20 bg-white/5 p-4">
            <p className="text-sm font-black">Cursos indisponíveis agora</p>
            <p className="mt-1 text-[11px] font-semibold text-slate-300">Tente atualizar para consultar seus acessos.</p>
          </div>
        ) : (
      ...
      <section className="rounded-[1.5rem] border border-slate-200/80 bg-white p-4 shadow-sm">
        ...
        {loadingProximosEventos ? (
          ...
          <div className="mt-4 flex items-center gap-3 rounded-2xl bg-amber-50 p-3">
            ...
            <p className="text-[11px] font-semibold text-amber-800">Não foi possível consultar sua agenda.</p>
          </div>
        ) : (
          <div className="mt-4 flex items-center gap-3 rounded-2xl bg-slate-50 p-3">
      ...
      >
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl ${
          financeDataError ? 'bg-white text-amber-600' : hasOverduePayment ? 'bg-white text-rose-600' : hasOpenPayment ? 'bg-blue-50 text-blue-600' : 'bg-white text-emerald-600'
        }`}>
          {financeDataError || hasOverduePayment ? <AlertTriangle size={20} /> : hasOpenPayment ? <WalletCards size={20} /> : <CheckCircle2 size={20} />}
        </div>
        <div className="min-w-0 flex-1">
