...
const CertificateTab: React.FC<CertificateTabProps> = ({ certificate, state, eadProgress }) => (
  <div className="min-w-0 rounded-2xl border border-slate-100 bg-slate-50/60 p-4 sm:p-6">
    ...
    {!state.isLoading && !state.isError && certificate?.status === 'FINALIZADO' && certificate.codigo_validacao ? (
      <div className="flex flex-col justify-between gap-5 lg:flex-row lg:items-center"><div className="min-w-0"><div className="mb-3 flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-600 text-white"><Award size={24} /></div><p className="text-[10px] font-black uppercase tracking-widest text-emerald-700">Certificado liberado</p><h4 className="mt-1 text-base font-black uppercase tracking-tight text-[#001a33] sm:text-lg">Curso concluído e certificado emitido</h4><p className="mt-2 break-words text-xs font-bold text-slate-500">Código de validação: <span className="break-all font-mono text-blue-700">{certificate.codigo_validacao}</span></p><p className="mt-1 text-xs font-semibold text-slate-500">Conclusão: {formatDate(certificate.data_conclusao)} • Nota final: {certificate.nota_final ?? getQuizScore(eadProgress) ?? '--'}</p></div><button type="button" onClick={() => window.open(getDocumentValidationUrl(certificate.codigo_validacao!), '_blank', 'noopener,noreferrer')} className="inline-flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-[#001a33] px-4 py-3 text-[10px] font-black uppercase tracking-widest text-white hover:bg-slate-800 sm:w-auto"><FileCheck2 size={14} /> Validar QR/Código</button></div>
    ) : null}
