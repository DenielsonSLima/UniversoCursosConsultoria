        ...
        return (
          <div key={discipline.id} className="rounded-2xl border border-slate-100 bg-white p-4">
            ...
            {disciplineClasses.length === 0 ? <p className="text-xs text-slate-500">Nenhuma aula registrada nesta disciplina.</p> : (
              <div className="divide-y divide-slate-100 rounded-xl border border-slate-100">{disciplineClasses.map((lesson) => { const status = attendanceByClass.get(String(lesson.id)); const sessionLabel = lesson.sessao === 'M' ? 'Manhã' : lesson.sessao === 'T' ? 'Tarde' : lesson.sessao === 'N' ? 'Noite' : null; return <div key={lesson.id} className="flex flex-col gap-2 p-3 text-xs sm:flex-row sm:items-center sm:justify-between"><p className="font-semibold text-[#001a33]">{lesson.titulo || 'Aula sem título'}</p><div className="flex flex-wrap gap-2 text-slate-500"><span>{lesson.data_aula || 'sem data'}{sessionLabel ? ` • ${sessionLabel} (${lesson.carga_horaria || 0}h)` : ''}</span><span className={`rounded-full px-2 py-1 text-[10px] font-black uppercase tracking-wider ${status === 'J' ? 'bg-amber-50 text-amber-700' : 'bg-slate-50'}`}>{status === 'P' ? 'presente' : status === 'F' ? 'falta' : status === 'J' ? 'falta justificada' : 'sem chamada'}</span></div></div>; })}</div>
            )}
          </div>
        );
      }) : null}
