      ...
      {!hasError ? disciplines.map((discipline) => {
        ...
        return (
          <div key={discipline.id} className="rounded-2xl border border-slate-100 bg-white p-4">
            <div className="mb-3 flex items-center justify-between gap-3"><h5 className="text-sm font-bold text-[#001a33]">{discipline.nome}</h5><span className="rounded-full border border-slate-100 bg-slate-50 px-3 py-1 text-[10px] uppercase tracking-widest text-slate-500">{discipline.frequency === null ? 'Frequência não lançada' : `${discipline.frequency}%`}</span></div>
