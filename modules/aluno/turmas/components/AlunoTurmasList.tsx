...
const AlunoTurmasList: React.FC<AlunoTurmasListProps> = ({
  ...
  const renderCard = (matricula: MatriculaAluno) => {
    ...
    return (
      <article key={matricula.id} className={`relative overflow-hidden rounded-[1.5rem] border bg-white shadow-sm transition-all duration-300 hover:border-blue-400 hover:shadow-md ${isPinned ? 'border-blue-300 ring-2 ring-blue-100' : 'border-slate-100'}`}>
        <button
          type="button"
          onClick={() => onTogglePinned(matricula)}
          className={`absolute right-4 top-4 z-10 flex h-10 w-10 items-center justify-center rounded-2xl border shadow-sm backdrop-blur transition ${isPinned ? 'border-blue-200 bg-blue-600 text-white hover:bg-blue-700' : 'border-white/70 bg-white/90 text-slate-500 hover:border-blue-200 hover:text-blue-700'}`}
        ...
        <div className="space-y-4 p-5">
          ...
          <button
            ...
            className="flex w-full items-center justify-center gap-2 rounded-xl bg-slate-50 py-3 text-[10px] font-black uppercase tracking-widest text-slate-600 transition-all hover:bg-blue-600 hover:text-white"
          >
  ...
  return (
    <div className="space-y-6">
      <div className="relative ml-auto w-full lg:w-80">
        <input type="search" placeholder="Buscar em meus cursos..." value={searchTerm} onChange={(event) => setSearchTerm(event.target.value)} className="w-full rounded-xl border border-slate-200 bg-white py-3 pl-9 pr-3 text-xs font-bold text-slate-700 shadow-sm outline-none transition-all focus:border-blue-500" />
        <Search size={14} className="absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
      </div>

      <div className="flex flex-wrap gap-2 rounded-2xl border border-slate-100 bg-white p-2 shadow-sm">
        {MODALITY_FILTERS.map((filter) => <button key={filter.id} type="button" onClick={() => setModalityFilter(filter.id)} className={`rounded-xl px-4 py-2 text-[10px] font-black uppercase tracking-widest transition-colors ${modalityFilter === filter.id ? 'bg-blue-600 text-white' : 'bg-slate-50 text-slate-500 hover:bg-blue-50 hover:text-blue-600'}`}>{filter.label}</button>)}
      </div>

      {filtered.length === 0 ? (
        <div className="rounded-[2rem] border border-slate-100 bg-white p-10 text-center shadow-sm"><p className="text-sm font-black text-[#001a33]">Nenhum curso encontrado</p><p className="mt-1 text-xs font-bold text-slate-400">Ajuste a busca ou selecione outro tipo de curso.</p></div>
      ) : (
        <>
