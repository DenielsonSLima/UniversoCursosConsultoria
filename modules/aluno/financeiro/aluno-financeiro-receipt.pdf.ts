...
import type { AlunoFinancialReceiptPayload } from './financeiro.types.ts';
...
export interface AlunoFinancialReceiptPreviewItem extends CanonicalDocumentPreviewItem {
  receiptPayload: AlunoFinancialReceiptPayload;
}

interface PreparedReceipt {
  item: AlunoFinancialReceiptPreviewItem;
...
const assertCompatiblePayload = (payload: AlunoFinancialReceiptPayload) => {
  if (payload.model.key !== 'recibo'
      || payload.model.source !== 'MODELO_RECIBO_PADRAO'
      || payload.model.documentKind !== 'RECIBO_PAGAMENTO_ALUNO') {
    throw new Error('O servidor retornou um modelo incompatível com o recibo do aluno.');
  }
...
const prepareReceipt = async (
  ...
  if (item.receiptPayload.institution.logoUrl && !logo) {
    throw new Error('Não foi possível resolver a logo configurada no cabeçalho institucional.');
  }
...
const drawWatermark = (
  ...
  const scale = Math.min(100, Math.max(18, watermark.scale)) / 100;
...
const drawField = (
  ...
  pdf.setFontSize(8.2);
  drawCanonicalPdfText(pdf, value || 'Não informado', x, y + 6.5, {
    maxWidth: width - 6,
    maxLines: 2,
    lineHeight: 1.12,
  });
...
const drawFieldGrid = (
  pdf: jsPDF,
  payload: AlunoFinancialReceiptPayload,
  startY: number,
) => {
  const { receipt } = payload;
  const rows = [
    [
      ['Aluno / pagador', receipt.payerName],
      ['Documento', receipt.payerDocument || 'Não informado'],
    ],
    [
  ...
  rows.forEach((row, index) => {
    ...
    pdf.roundedRect(LEFT, y, CONTENT_WIDTH, rowHeight - 2, 1.8, 1.8, 'FD');
    drawField(pdf, row[0][0], row[0][1], LEFT + 3, y, columnWidth);
    pdf.line(LEFT + columnWidth + gap / 2, y + 2, LEFT + columnWidth + gap / 2, y + 13);
    drawField(pdf, row[1][0], row[1][1], LEFT + columnWidth + gap, y, columnWidth);
  });
};
...
const drawReceiptPage = (
  ...
  const header = drawCanonicalInstitutionalHeader(
    ...
    prepared.logo,
    {
      ...
      alias: `student-receipt-logo-${receipt.id}`,
      meta: {
        eyebrow: 'Financeiro do aluno',
        title: receipt.title,
        label: 'Situação',
  ...
  pdf.roundedRect(LEFT, valueTop, CONTENT_WIDTH, 24, 3, 3, 'FD');
  pdf.setTextColor(100, 116, 139);
  pdf.setFontSize(5.8);
  ...
  pdf.setTextColor(0, 26, 51);
  pdf.setFontSize(20);
  pdf.text(currencyFormatter.format(receipt.valuePaid), PAGE_WIDTH / 2, valueTop + 9.5, {
    align: 'center', baseline: 'top',
  });
  ...
  pdf.setTextColor(15, 23, 42);
  pdf.setFontSize(10);
  drawCanonicalPdfText(pdf, normalizeCanonicalPdfText(receipt.declaration), LEFT, narrativeTop, {
  ...
  pdf.roundedRect(LEFT, descriptionTop, CONTENT_WIDTH, 21, 2.5, 2.5, 'FD');
  pdf.setFont('helvetica', 'bold');
  pdf.setTextColor(29, 78, 216);
  pdf.setFontSize(5.8);
  pdf.text('REFERÊNCIA', LEFT + 4, descriptionTop + 3.2, { baseline: 'top' });
  pdf.setTextColor(30, 58, 138);
  pdf.setFontSize(8.4);
  drawCanonicalPdfText(
    pdf,
    `${receipt.description}${receipt.courseLabel ? ` · ${receipt.courseLabel}` : ''}`,
    LEFT + 4,
    descriptionTop + 8,
    { maxWidth: CONTENT_WIDTH - 8, maxLines: 2, lineHeight: 1.2 },
  );
  ...
  pdf.line(PAGE_WIDTH - 98, signatureTop + 18, PAGE_WIDTH - RIGHT, signatureTop + 18);
  pdf.setTextColor(0, 26, 51);
  pdf.setFontSize(7.2);
  ...
  pdf.setDrawColor(226, 232, 240);
  pdf.line(LEFT, footerY - 3, PAGE_WIDTH - RIGHT, footerY - 3);
  pdf.setTextColor(100, 116, 139);
  pdf.setFontSize(5.2);
  pdf.text(receipt.footerNote, LEFT, footerY - 0.2, {
    baseline: 'top', maxWidth: CONTENT_WIDTH - 45,
  });
  pdf.text(`Emitido em ${receipt.emittedAtLabel}`, PAGE_WIDTH - RIGHT, footerY, {
    align: 'right', baseline: 'top',
  });
};

export const createAlunoFinancialReceiptPdf = async (
  items: readonly AlunoFinancialReceiptPreviewItem[],
  ...
  return {
    ...
    fileName: `recibo-aluno-${first.receiptNumber.toLowerCase()}.pdf`,
  };
};
