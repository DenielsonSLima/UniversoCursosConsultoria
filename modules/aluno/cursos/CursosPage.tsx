...
import { BookOpen, MonitorPlay, Zap, Award, Clock, ArrowUpRight, Search } from 'lucide-react';
...
const CursosPage: React.FC = () => {
  ...
  const { data: courses = [], isLoading, isError } = useQuery<any[]>({
    queryKey: ['aluno-cursos-disponiveis'],
    queryFn: async () => {
  ...
  const currentTabCourses = getFilteredCourses();
    ...
    <div className="space-y-6 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          ...
          <p className="text-xs text-slate-450 font-medium">Explore novas oportunidades acadêmicas e especializações</p>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {currentTabCourses.map(course => (
            <div 
              key={course.id}
              className="bg-white rounded-[2.5rem] border border-slate-100 hover:border-blue-500 shadow-sm hover:shadow-md transition-all duration-300 p-6 flex flex-col justify-between group"
            >
              <div className="space-y-4">
                {/* Accent icon */}
                <div className="flex justify-between items-center">
                  <div className={`w-10 h-10 rounded-2xl flex items-center justify-center ${
                    activeTab === 'ead' ? 'bg-blue-50 text-blue-600' :
                    activeTab === 'live' ? 'bg-amber-50 text-amber-600' : 'bg-purple-50 text-purple-600'
                  }`}>
                ...
                <div>
                  ...
                  <p className="text-[11px] text-slate-500 font-medium line-clamp-3 mt-2 leading-relaxed">
                    {course.descricao || 'Sem descrição cadastrada para este curso no momento. Saiba mais falando com nossa secretaria.'}
                  </p>
                </div>
              </div>
