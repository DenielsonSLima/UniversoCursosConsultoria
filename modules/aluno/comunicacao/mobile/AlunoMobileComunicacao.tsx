...
const AlunoMobileComunicacao = ({
  ...
  if (showConversation && currentChat) {
    ...
    return (
      <section className="flex h-[calc(100dvh-10.5rem-env(safe-area-inset-top)-env(safe-area-inset-bottom))] min-h-[420px] flex-col overflow-hidden rounded-[1.75rem] border border-slate-200 bg-white shadow-sm animate-fadeIn">
        <header className="flex shrink-0 items-center gap-2 border-b border-slate-200 bg-white px-3 py-3 shadow-sm">
          <button type="button" onClick={onBack} aria-label="Voltar para chamados" className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-slate-100 text-[#001a33] active:scale-95">
            <ArrowLeft size={20} />
          </button>
          <div className="min-w-0 flex-1">
            <h2 className="truncate text-sm font-black text-[#001a33]">Suporte Universo</h2>
            <div className="mt-1 flex min-w-0 items-center gap-2">
              <span className="flex min-w-0 items-center gap-1 truncate text-[9px] font-black uppercase tracking-wider text-slate-500">
                <Tag size={10} className="shrink-0" /> {category.nome}
              </span>
              <span className={`shrink-0 rounded-full px-2 py-1 text-[8px] font-black uppercase tracking-wider ${isPending ? 'bg-amber-50 text-amber-700' : 'bg-emerald-50 text-emerald-700'}`}>
        ...
        <div className="shrink-0 border-t border-slate-200 bg-white">
          {currentChat.status === 'solucionada' ? (
            <div className="p-3">
              <div className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50 px-3 text-center text-[9px] font-black uppercase tracking-wider text-emerald-700">
    ...
    <section className="space-y-4 animate-fadeIn">
      <header className="relative overflow-hidden rounded-[1.75rem] bg-[#001a33] px-5 py-5 text-white shadow-lg shadow-slate-900/10">
        ...
        <div className="relative flex items-start justify-between gap-4">
          <div>
            <p className="text-[9px] font-black uppercase tracking-[0.24em] text-blue-300">Fale com a Universo</p>
        ...
        <button type="button" onClick={onNewChat} className="relative mt-5 flex min-h-12 w-full items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 text-[10px] font-black uppercase tracking-[0.16em] text-white shadow-lg shadow-blue-950/30 active:scale-[0.99]">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-1 shadow-sm" role="tablist" aria-label="Filtrar chamados">
        <div className="grid grid-cols-2 gap-1">
          <button type="button" role="tab" aria-selected={activeCallTab === 'pendentes'} onClick={() => onTabChange('pendentes')} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors ${activeCallTab === 'pendentes' ? 'bg-amber-50 text-amber-700 shadow-sm' : 'text-slate-500'}`}>
            <Clock size={14} /> Em aberto <span className="rounded-full bg-white px-1.5 py-0.5 text-[9px] shadow-sm">{pendingCount}</span>
          </button>
          <button type="button" role="tab" aria-selected={activeCallTab === 'resolvidos'} onClick={() => onTabChange('resolvidos')} className={`flex min-h-11 items-center justify-center gap-2 rounded-xl text-[10px] font-black uppercase tracking-wider transition-colors ${activeCallTab === 'resolvidos' ? 'bg-emerald-50 text-emerald-700 shadow-sm' : 'text-slate-500'}`}>
            <CheckCircle size={14} /> Resolvidos <span className="rounded-full bg-white px-1.5 py-0.5 text-[9px] shadow-sm">{resolvedCount}</span>
          </button>
        </div>
      </div>
      ...
      <div className="space-y-2.5">
        {loadingChats ? (
          ...
          return (
            <button key={chat.id} type="button" onClick={() => onSelectChat(chat.id)} className="flex min-h-[92px] w-full items-start gap-3 rounded-2xl border border-slate-200 bg-white p-4 text-left shadow-sm transition active:scale-[0.99] active:border-blue-200">
              ...
              <div className="min-w-0 flex-1">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <time className="shrink-0 text-[9px] font-bold text-slate-400">{formatChatTime(chat.ultima_data)}</time>
                </div>
                <p className="mt-1 truncate text-xs font-medium text-slate-500">{chat.ultimo_texto || 'Aguardando mensagem...'}</p>
                <div className="mt-2 flex items-center gap-2 text-[8px] font-black uppercase tracking-wider">
