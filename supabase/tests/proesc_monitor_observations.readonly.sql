...
begin
  ...
  v_page:=public.get_proesc_reconciliation_feed_page(p_context=>'observations',p_page_size=>10);
  ...
  for v_item in select value from jsonb_array_elements(v_page->'items') loop
    ...
    assert (v_item->>'poloId')::uuid=any(public.gestor_allowed_polo_ids()),
      'Observation outside authorized polo scope';
    if v_item->>'verification'='REVIEW' then
      assert v_item->'receivedAmount' is null or v_item->'receivedAmount'='null'::jsonb,
        'Unverified observation asserted a received amount';
    end if;
  end loop;
  perform set_config('app.proesc_monitor_test_result',jsonb_build_object('unit','proesc_monitor_observations.readonly','passed',true,'durationMs',coalesce(v_duration_ms,round(extract(epoch from clock_timestamp()-v_started)*1000)))::text,true);
end;
$monitor_check$;
