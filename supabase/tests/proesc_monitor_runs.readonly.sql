...
begin
  ...
  v_page:=public.get_proesc_reconciliation_feed_page(p_context=>'runs',p_page_size=>10);
  ...
  for v_item in select value from jsonb_array_elements(v_page->'items') loop
    ...
    assert v_item->>'status' in ('RUNNING','SUCCEEDED','PARTIAL','FAILED','ABANDONED'),
      'Unknown execution outcome';
    if v_item->>'status'='ABANDONED' then
      assert v_item->'finishedAt'='null'::jsonb and v_item->'durationMs'='null'::jsonb,
        'Abandoned execution has invented finish or duration';
    end if;
  end loop;
  perform set_config('app.proesc_monitor_test_result',jsonb_build_object('unit','proesc_monitor_runs.readonly','passed',true,'durationMs',coalesce(v_duration_ms,round(extract(epoch from clock_timestamp()-v_started)*1000)))::text,true);
end;
$monitor_check$;
