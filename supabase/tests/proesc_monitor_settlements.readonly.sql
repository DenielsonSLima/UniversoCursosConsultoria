...
begin
  ...
  v_page:=public.get_proesc_reconciliation_feed_page(p_context=>'settlements',p_page_size=>10);
  ...
  for v_item in select value from jsonb_array_elements(v_page->'items') loop
    ...
    assert v_item->>'result'='APPLIED','Unchanged title presented as settlement';
    assert exists(select 1 from internal_proesc.reconciliation_events e
      join internal_proesc.financial_snapshots s on s.id=e.snapshot_id
      where e.id=(v_item->>'id')::uuid and e.result='APPLIED' and e.after_state->>'status'='PAGO'
        and s.source_status='PAID' and s.verification='VERIFIED'),
      'Settlement lacks actual paid proof';
  end loop;
  perform set_config('app.proesc_monitor_test_result',jsonb_build_object('unit','proesc_monitor_settlements.readonly','passed',true,'durationMs',coalesce(v_duration_ms,round(extract(epoch from clock_timestamp()-v_started)*1000)))::text,true);
end;
$monitor_check$;
