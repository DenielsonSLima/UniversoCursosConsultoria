-- Pagina as tentativas, baixas e erros da Consulta API Banese em lotes de 20
-- para otimizar consumo de egress e carregamento sob demanda.
...
revoke all on function public.get_banese_reconciliation_attempts_page(text, integer, integer)
  from public, anon;
grant execute on function public.get_banese_reconciliation_attempts_page(text, integer, integer)
  to authenticated, service_role;

comment on function public.get_banese_reconciliation_attempts_page(text, integer, integer) is
  'Retorna histórico paginado (20 por página) das consultas, baixas e erros Banese, protegendo egress e separando os dados financeiros autorizados.';

-- Otimiza get_banese_reconciliation_dashboard para não trafegar listas pesadas a cada ciclo de polling
...
begin
  ...
  select jsonb_build_object(
    ...
    'lastAttempts', '[]'::jsonb,
    'lastSettlements', '[]'::jsonb,
    'lastErrorAttempts', '[]'::jsonb,
    'transitions', (
      select coalesce(
...
comment on function public.get_banese_reconciliation_dashboard() is
  'Painel Banese protegido: estado e estatísticas leves sem trafegar listas históricas inteiras.';
