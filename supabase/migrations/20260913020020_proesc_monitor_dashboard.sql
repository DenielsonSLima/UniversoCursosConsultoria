begin;
create function internal_proesc.monitor_scope(p_polo_id uuid)
returns uuid[] language plpgsql stable security definer set search_path='' as $$
declare v_polos uuid[];
begin
  if auth.uid() is null or not public.is_gestor_global() or not public.gestor_has_module('configuracoes') then
    raise exception 'Acesso negado à Consulta API Proesc.' using errcode='42501'; end if;
  ...
  if cardinality(v_polos)=0 or (p_polo_id is not null and not p_polo_id=any(v_polos)) then
    raise exception 'Polo fora do escopo autorizado.' using errcode='42501'; end if;
  return case when p_polo_id is null then v_polos else array[p_polo_id] end;
end;
$$;
create function internal_proesc.monitor_period(p_from timestamptz,p_to timestamptz)
returns table(started_from timestamptz,started_to timestamptz)
language plpgsql stable set search_path='' as $$
begin
  started_to:=coalesce(p_to,now()); started_from:=coalesce(p_from,started_to-interval '31 days');
  if started_from>started_to or started_to-started_from>interval '31 days 1 second' then
    raise exception 'Escolha um período de até 31 dias.' using errcode='22023'; end if;
  return next;
end;
$$;
create function internal_proesc.monitor_links(p_polos uuid[])
...
language sql stable security definer set search_path='' as $$
  ...
  where c.polo_id=any(p_polos) and internal_proesc.reconciliation_source_system(c)='PROESC';
$$;

create function public.get_proesc_reconciliation_dashboard(
...
begin
  ...
  events as materialized(select e.mode,e.result from internal_proesc.reconciliation_events e
    join links l on l.link_id=e.link_id where e.recorded_at>=v_from and e.recorded_at<=v_to
...
$$;
revoke all on function internal_proesc.monitor_scope(uuid),internal_proesc.monitor_period(timestamptz,timestamptz),
  internal_proesc.monitor_links(uuid[]) from public,anon,authenticated,service_role;
revoke all on function public.get_proesc_reconciliation_dashboard(uuid,timestamptz,timestamptz) from public,anon,authenticated,service_role;
grant execute on function public.get_proesc_reconciliation_dashboard(uuid,timestamptz,timestamptz) to authenticated;
