-- O fluxo de Calendário de Aulas ganha filtro por módulo APENAS para cursos
-- técnicos. O restante do fluxo continua canônico: mês, autorização e
-- composição de payload permanecem no banco.

create or replace function public.listar_modulos_calendario_aulas_secure(
  p_polo_id uuid,
  p_turma_id uuid
)
returns jsonb
...
begin
  ...
  if not exists (
    ...
    raise exception 'Turma incompatível com o polo selecionado.' using errcode = '42501';
  end if;

  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        'modulo_id', modulo_set.modulo_id,
        'modulo_nome', modulo_set.modulo_nome,
        'modulo_ordem', modulo_set.modulo_ordem
      ) order by modulo_set.modulo_ordem nulls last, modulo_set.modulo_nome
    ),
    '[]'::jsonb
  )
  into v_modulos
  from (
    select distinct
      ...
      trim(both from modulo.nome) as modulo_nome,
...
begin
  ...
  if p_modulo_id is not null and v_modalidade <> 'TECNICO' then
    raise exception 'A filtragem por módulo só é permitida para cursos Técnicos.' using errcode = '22023';
  end if;

  if p_modulo_id is not null and not exists (
    select 1
    from public.modulos m
    where m.id = p_modulo_id
  ) then
    raise exception 'Módulo não encontrado.' using errcode = '22023';
  end if;
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'data_exibicao', to_char(calendar_line.data_aula, 'DD/MM/YYYY'),
  ...
  from (
    ...
    where class_meeting.turma_id = p_turma_id
      and (p_modulo_id is null or module.id = p_modulo_id)
    ...
    'documento', jsonb_build_object(
      ...
      'subtitulo', replace(
        replace(
          ...
          '{{TURMA}}', coalesce(nullif(v_turma.turma_codigo, ''), v_turma.turma_nome)
        ),
...
-- Compatibilidade com chamadas antigas (sem filtro de módulo).
create or replace function public.preparar_calendario_aulas_exportacao_secure(
  p_polo_id uuid,
  p_modalidade text,
  p_turma_id uuid
...
revoke all on function public.listar_modulos_calendario_aulas_secure(uuid, uuid)
  from public, anon;
grant execute on function public.listar_modulos_calendario_aulas_secure(uuid, uuid)
  to authenticated, service_role;

revoke all on function public.preparar_calendario_aulas_exportacao_secure(
  uuid,
  text,
  uuid,
  date,
  ...
  from public, anon;
grant execute on function public.preparar_calendario_aulas_exportacao_secure(
  uuid,
  text,
  uuid,
  date,
