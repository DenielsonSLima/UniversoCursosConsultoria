-- Reserva títulos Banese não pagos, vencidos ou não, com snapshot e CAS.
...
begin
  ...
  with candidates as (
    select
      ...
      regexp_replace(c.gateway_boleto_convenio, '\D', '', 'g')
        as current_convenio,
...
revoke all on function public.start_banese_cancellation_remote_attempt(
  uuid, uuid
) from public, anon, authenticated;
grant execute on function public.start_banese_cancellation_remote_attempt(
  uuid, uuid
) to service_role;
