...
CREATE TEMP TABLE technical_interest_correction_guard
ON COMMIT DROP
AS
SELECT
...
-- Política correta:
--   multa única: 2% = R$ 5,60 sobre R$ 279,90;
--   um único juros: 1% ao mês, proporcional por dia;
--   referência diária: 0,033333% e aproximadamente R$ 0,09 por dia.
UPDATE public.turmas AS turma
SET juros_atraso = 1.00
...
COMMENT ON FUNCTION public.aplicar_padrao_financeiro_turma_tecnica() IS
  'Aplica 150 + 12x279,90 + rematrícula 150, multa única de 2% e um juros de 1% ao mês proporcional por dia, sem gateway.';
...
DECLARE
  ...
  v_preview RECORD;
  v_guard technical_interest_correction_guard%ROWTYPE;
BEGIN
  SELECT COUNT(*), COUNT(*) FILTER (
    WHERE turma.juros_atraso <> 1.00
  ...
  IF v_classes <= 0 OR v_invalid_classes <> 0 THEN
    RAISE EXCEPTION
      'Validação dos juros técnicos falhou: % turmas, % inválidas.',
      v_classes,
      v_invalid_classes;
  END IF;
  ...
  IF v_preview.valor_com_desconto <> 260.00
     OR v_preview.juros_calculados <> 2.80
  ...
  THEN
    RAISE EXCEPTION
      'Prévia técnica de 1%% ao mês inválida: %.',
      TO_JSONB(v_preview);
  END IF;

  SELECT *
  INTO v_guard
  FROM technical_interest_correction_guard;
  ...
  THEN
    RAISE EXCEPTION
      'A correção de juros tentou alterar recebíveis ou identidades de gateway.';
  END IF;
END;
$$;
