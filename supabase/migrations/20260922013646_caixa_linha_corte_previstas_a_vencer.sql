-- Campo aditivo canônico: preserva a fórmula financeira antes exibida no JSX.
-- Sem novo arredondamento; todas as demais regras, filtros e guardas são mantidos.
SET LOCAL lock_timeout = '3s';
SET LOCAL statement_timeout = '5s';
DO $baseline$
BEGIN
  IF encode(extensions.digest(pg_get_functiondef(
    'public.get_caixa_linha_corte_secure(uuid,date)'::regprocedure), 'sha256'), 'hex') <>
    '0d097d6d5cce946b314ba9a4294f915d0ed255fc682cb1ca6dfb2d522a8dc9ba' THEN
    RAISE EXCEPTION 'Linha de corte changed; review the canonical field migration.';
  END IF;
END;
$baseline$;
...
BEGIN
  ...
  RETURN jsonb_build_object(
    ...
    'receitas', jsonb_build_object(
      ...
      'previstas_a_vencer', greatest(0::numeric, v_receitas_previstas - v_inadimplencia_vencida),
