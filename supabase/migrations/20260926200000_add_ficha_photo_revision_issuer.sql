-- Novas versões de Ficha podem mudar a foto sem alterar códigos ou snapshots
-- anteriores. A Pasta conserva seu contrato de emissão e apresentação.
begin;

create or replace function internal_academic.ficha_student_photo_reference(p_photo text)
returns text
language sql
immutable
set search_path = ''
as $function$
  select 'student-photo:' || encode(extensions.digest(convert_to(
    case
      when btrim(coalesce(p_photo, '')) ~* '(^|/)sem-foto-aluno\.svg([?#].*)?$' then ''
      else btrim(coalesce(p_photo, ''))
    end, 'UTF8'), 'sha256'), 'hex');
$function$;

revoke all on function internal_academic.ficha_student_photo_reference(text)
...
begin
  ...
  if v_referencia is not null then
    if p_documento <> 'ficha_matricula'
      or v_referencia !~ '^student-photo:[0-9a-f]{64}$'
    then
    ...
    ) and v_referencia <> internal_academic.ficha_student_photo_reference(
      v_enrollment.student_photo_url
    ) then
      raise exception 'A referência da foto diverge do cadastro atual.'
        using errcode = '22023';
    end if;
  end if;
...
-- Mantém a assinatura pública anterior e todas as suas guardas internas.
create or replace function public.emitir_ficha_validacao_portal(
  p_documento text, p_matricula_id uuid, p_periodo_referencia text default null,
  p_emitido_por uuid default null, p_registrar_reemissao boolean default false,
...
as $function$
  ...
  from internal_academic.emitir_ficha_validacao_com_referencia(
    p_documento, p_matricula_id, p_periodo_referencia, p_emitido_por,
    p_registrar_reemissao, p_dados_emissao, null
  ) as issued;
$function$;
