...
create or replace function public.get_diarios_turma_com_historico(p_turma_id uuid)
returns jsonb
...
as $function$
  ...
  select coalesce(jsonb_agg(
    (to_jsonb(c) - 'ordinality') || jsonb_build_object('historico',
      case when i.id is not null then jsonb_build_object(
        ...
        'readOnly', mat.importacao_id is null,
        'materialization',jsonb_build_object('status',case when mat.importacao_id is null then 'PENDENTE' else 'MATERIALIZADO' end,'completedAt',mat.completed_at),
        'estado',case when mat.importacao_id is null then 'EM_CONFERENCIA' else 'MATERIALIZADO' end,
  ...
  left join internal_academic.diario_materializacoes mat on mat.importacao_id=i.id
