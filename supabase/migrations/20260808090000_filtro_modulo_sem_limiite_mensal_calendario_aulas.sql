-- Para exportação por módulo técnico, o recorte mensal é removido para manter
-- a semântica histórica do PDF do módulo: todas as aulas do módulo selecionado,
-- passadas e futuras, em um único documento.
...
declare
  ...
  v_fim_periodo date;
  v_aplica_recorte_mensal boolean;
  v_turma record;
...
begin
  ...
  v_aplica_recorte_mensal := p_modulo_id is null;
  ...
  from (
    ...
    where class_meeting.turma_id = p_turma_id
      ...
      and (
        not v_aplica_recorte_mensal
        or (class_meeting.data_aula >= v_inicio_periodo and class_meeting.data_aula < v_fim_periodo)
      )
    group by
      subject.nome,
      class_meeting.data_aula,
  ...
  if jsonb_array_length(v_linhas) > 0 then
    ...
    v_mensagem := case
      when v_aplica_recorte_mensal
        then 'Esta turma EAD não possui aulas datadas no mês selecionado.'
      else 'Esta turma EAD não possui aulas datadas para o módulo selecionado.'
    end;
  elsif not v_aplica_recorte_mensal then
    v_status := 'SEM_GRADE';
    v_mensagem := 'Não há aulas registradas para este módulo.';
