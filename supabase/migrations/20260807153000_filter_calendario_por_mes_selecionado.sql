-- O período da exportação é canônico: recebe o mês ativo da agenda, limita a
-- grade àquele mês e exclui aulas que já transcorreram. O navegador não pode
-- filtrar/ordenar nem reintroduzir linhas fora desse intervalo.
...
begin
  ...
  v_inicio_periodo := greatest(v_mes_referencia, current_date);
  ...
  if jsonb_array_length(v_linhas) > 0 then
    ...
    v_mensagem := 'Esta turma EAD não possui aulas datadas restantes no mês selecionado.';
  elsif v_inicio_periodo >= v_fim_periodo then
    v_status := 'SEM_GRADE';
    v_mensagem := 'O mês selecionado já não possui aulas futuras para exportar.';
  else
    v_status := 'SEM_GRADE';
    v_mensagem := 'Não há aulas restantes no mês selecionado para esta turma.';
  end if;
