-- O núcleo acadêmico passa a corrigir pelo cofre privado e nunca sobrescreve
-- uma matrícula que deixou de estar ativa durante a conclusão.

CREATE OR REPLACE FUNCTION public.ead_update_aluno_progress_core_20260822(
  p_aluno_id uuid,
  p_curso_id uuid,
  p_action text,
...
BEGIN
  ...
  SELECT internal_academic.ead_restore_assessment_answers(
    c.ead_config,
    k.activity_answers,
    k.quiz_answers
  )
  ...
  v_retry_hours := greatest(
    coalesce((v_regras ->> 'intervaloReprovacaoHoras')::integer, 3),
    1
  );
  ...
  THEN
    RAISE EXCEPTION 'Informe o item do conteúdo EAD.' USING ERRCODE = '23514';
  END IF;
  ...
  IF p_action = 'toggle_content' THEN
    ...
    END IF;
    IF coalesce(v_progress -> 'completedContentIds', '[]'::jsonb) ? p_item_id THEN
  ...
  ELSIF p_action = 'toggle_activity' THEN
    ...
    IF NOT v_is_completed THEN
      ...
      END IF;
      IF v_activity ? 'respostaCorreta'
        AND (
          v_answer !~ '^(0|[1-9][0-9]*)$'
          OR v_answer::numeric <> (v_activity ->> 'respostaCorreta')::numeric
        )
      THEN
  ...
  ELSIF p_action = 'toggle_video' THEN
    ...
    END IF;
    IF coalesce(v_progress -> 'completedVideoIds', '[]'::jsonb) ? p_item_id THEN
      RETURN public.ead_get_aluno_progress(p_aluno_id, p_curso_id);
    END IF;
    v_progress := jsonb_set(
      v_progress,
  ...
  ELSIF p_action = 'finish_quiz' THEN
    ...
    IF v_last_failed_at IS NOT NULL THEN
      ...
      THEN
        RAISE EXCEPTION 'Nova tentativa liberada somente após % horas da reprovação.',
          v_retry_hours USING ERRCODE = '23514';
      END IF;
    END IF;

    FOR q IN SELECT value FROM jsonb_array_elements(v_questions) LOOP
      IF coalesce(v_answers ->> (q ->> 'id'), '') ~ '^(0|[1-9][0-9]*)$'
        AND (v_answers ->> (q ->> 'id'))::numeric
          = (q ->> 'respostaCorreta')::numeric
      THEN
  ...
  IF public.ead_progress_meets_completion(v_progress, v_course_config) THEN
    ...
    IF upper(coalesce(v_matricula.status, '')) = 'ATIVO' THEN
      ...
      IF v_updated_rows <> 1 THEN
        RAISE EXCEPTION 'A matrícula EAD mudou de estado durante a conclusão.'
          USING ERRCODE = '40001';
      END IF;
    ELSIF upper(coalesce(v_matricula.status, '')) <> 'CONCLUIDO' THEN
      RAISE EXCEPTION 'A matrícula EAD não está elegível para conclusão.'
        USING ERRCODE = '23514';
    END IF;

    INSERT INTO public.certificados_academicos (
      matricula_id, aluno_id, turma_id, curso_id, polo_id, modalidade, status,
      data_inscricao, data_conclusao, nota_final, ensino_medio_estabelecimento,
      ensino_medio_localidade_uf, ensino_medio_ano_conclusao, metadados
    )
    VALUES (
      v_matricula.id,
      p_aluno_id,
  ...
  GET DIAGNOSTICS v_updated_rows = ROW_COUNT;
  IF v_updated_rows <> 1 THEN
    RAISE EXCEPTION 'O progresso EAD mudou durante a atualização.' USING ERRCODE = '40001';
  END IF;
...
REVOKE ALL ON FUNCTION public.ead_update_aluno_progress_core_20260822(
  uuid, uuid, text, text, jsonb
) FROM PUBLIC, anon, authenticated, service_role;

COMMENT ON FUNCTION public.ead_update_aluno_progress_core_20260822(
  uuid, uuid, text, text, jsonb
) IS 'Núcleo privado EAD: corrige com gabarito interno e conclui matrícula sob lock condicional.';
