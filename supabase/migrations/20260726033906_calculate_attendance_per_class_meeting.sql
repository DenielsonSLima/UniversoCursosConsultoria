-- Frequência acadêmica é calculada por encontro lançado, não pela carga
-- horária do encontro. Assim, em quatro encontros, cada um representa 25%.

create or replace function internal_academic.calculate_attendance_percent_by_meeting(
  p_total_meetings bigint,
  p_recorded_meetings bigint,
  p_absences bigint
)
returns numeric
language sql
...
as $function$
  select case
    when coalesce(p_total_meetings, 0) <= 0 then null
    when coalesce(p_recorded_meetings, 0) <> p_total_meetings then null
    else round(
      least(
        100::numeric,
        greatest(
          ...
          (
            (
              p_total_meetings - coalesce(p_absences, 0)
            )::numeric / p_total_meetings::numeric
          ) * 100
        )
      ),
      2
...
revoke all on function internal_academic.calculate_attendance_percent_by_meeting(
  bigint,
  bigint,
  bigint
) from public, anon, authenticated;

comment on function internal_academic.calculate_attendance_percent_by_meeting(
  bigint,
  bigint,
  bigint
) is
  'Calcula frequência por encontro elegível. Retorna null enquanto houver frequência pendente.';
  ...
  base as (
    select
      ...
      case
        ...
        else internal_academic.calculate_attendance_percent_by_meeting(
          aulas.total,
          coalesce(frequencias.lancamentos, 0),
          coalesce(frequencias.faltas, 0)
        )
      end as frequencia,
      case
...
begin
  ...
  left join lateral (
    select round(
      internal_academic.calculate_attendance_percent_by_meeting(
        count(*),
        count(*),
        count(*) filter (where f.status = 'F')
      ),
      1
