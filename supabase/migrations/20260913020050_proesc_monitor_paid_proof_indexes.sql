...
-- Settlement eligibility is unchanged. Cover its immutable proof predicates so
-- counting a page does not fetch/decompress each event's complete after_state.
-- Event details are still read only for the selected page.
create index proesc_monitor_paid_events_cover_idx
  on internal_proesc.reconciliation_events(recorded_at desc,id desc)
  include(link_id,snapshot_id,mode)
  where result='APPLIED' and mode in ('AUTO','IMPORT','CORRECTION')
    and after_state->>'status'='PAGO';

-- The paired PAID/VERIFIED check needs only the snapshot ID. Do not read its
-- financial accounting payload just to prove membership in the paid set.
create index proesc_monitor_verified_paid_snapshot_idx
  on internal_proesc.financial_snapshots(id)
  where source_status='PAID' and verification='VERIFIED';

-- Dashboard exposes only APPLIED counts; push that existing condition into the
-- event scan so it can use the same covering index as the settlement feed.
