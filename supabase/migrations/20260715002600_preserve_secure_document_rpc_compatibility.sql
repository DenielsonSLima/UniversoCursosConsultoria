-- Mantém compatibilidade com o frontend já publicado sem reabrir as funções
-- internas. Os nomes antigos passam a apontar para os wrappers autorizados.

ALTER FUNCTION public.emitir_documento_validacao(
  text, uuid, text, text, timestamptz, uuid, boolean
) RENAME TO emitir_documento_validacao_interno;

ALTER FUNCTION public.revogar_documento_validacao(text)
  RENAME TO revogar_documento_validacao_interno;
...
REVOKE ALL ON FUNCTION public.revogar_documento_validacao_interno(text)
  FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.revogar_documento_validacao_interno(text)
  TO service_role;
...
BEGIN
  ...
  ELSIF v_is_owner THEN
    IF p_documento NOT IN (
      'carteirinha', 'cracha_estagio',
      'declaracao_matricula', 'declaracao_irpf'
    ) THEN
    ...
    IF p_documento = 'declaracao_irpf' THEN
      ...
      IF v_periodo IS NOT NULL THEN
        IF v_periodo !~ '^\d{4}$' THEN
          RAISE EXCEPTION 'Ano de referência do IRPF inválido.' USING ERRCODE = '22007';
        END IF;
  ...
  RETURN QUERY
  SELECT * FROM public.emitir_documento_validacao_interno(
    p_documento, p_matricula_id, v_periodo, v_referencia,
    v_validade, v_responsavel, v_registrar_reemissao
  );
END;
$$;
...
BEGIN
  SELECT dv.documento, dv.polo_id INTO v_document
  ...
  IF coalesce(auth.role(), '') <> 'service_role' AND NOT v_can_manage THEN
    RAISE EXCEPTION 'Sem permissão para revogar documento deste polo.'
      USING ERRCODE = '42501';
  END IF;
  RETURN public.revogar_documento_validacao_interno(p_codigo);
END;
$$;
...
RETURNS TABLE (
  ...
  ultima_emissao_em timestamptz, validade_ate timestamptz,
...
SET search_path = ''
AS $$
  SELECT * FROM public.emitir_documento_validacao_portal(
...
SET search_path = ''
AS $$
  SELECT public.revogar_documento_validacao_portal(p_codigo);
$$;
