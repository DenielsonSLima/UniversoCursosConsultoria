  ...
  if v_context='runs' then
    ...
    filtered as materialized(
      select s.id,s.link_id,s.observed_at from (
        ...
        union all
        select h.id,h.link_id,h.observed_at from internal_proesc.sync_run_items i
        cross join lateral(select h.id,h.link_id,h.observed_at
          from internal_proesc.financial_observation_history h
          where h.id=coalesce(i.original_snapshot_id,i.snapshot_id) offset 0) h
        where p_run_id is not null and i.run_id=p_run_id
    ...
    select (select count(*) from filtered),coalesce(jsonb_agg(jsonb_build_object(
      ...
      'runId',(select i.run_id from (select run_id from internal_proesc.sync_run_items
        where coalesce(original_snapshot_id,snapshot_id)=s.id offset 0) i
