...
begin
  ...
  set
    ...
    data_inicio = case
      when p_config ? 'data_inicio' then pg_catalog.nullif(p_config ->> 'data_inicio', '')::date
      else data_inicio
    end,
    data_previsao_termino = case
      when p_config ? 'data_previsao_termino' then pg_catalog.nullif(p_config ->> 'data_previsao_termino', '')::date
      else data_previsao_termino
    end,
    data_inicio_inscricao = case
      when p_config ? 'data_inicio_inscricao' then pg_catalog.nullif(p_config ->> 'data_inicio_inscricao', '')::date
      else data_inicio_inscricao
    end,
    data_fim_inscricao = case
      when p_config ? 'data_fim_inscricao' then pg_catalog.nullif(p_config ->> 'data_fim_inscricao', '')::date
    ...
    obs_financeira_origem = case
      when p_config ? 'obs_financeira_origem' then pg_catalog.nullif(p_config ->> 'obs_financeira_origem', '')
