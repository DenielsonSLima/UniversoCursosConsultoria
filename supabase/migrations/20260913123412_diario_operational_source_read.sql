...
declare v_import internal_academic.diario_importacoes; v_response jsonb; v_materialized internal_academic.diario_materializacoes;
begin
  ...
  select * into v_materialized from internal_academic.diario_materializacoes where importacao_id=v_import.id;
  select jsonb_build_object(
    'id',v_import.id,'sourceName',v_import.source_name,'sourceSha256',v_import.source_sha256,
    'importedAt',v_import.created_at,'readOnly',v_materialized.importacao_id is null,
    'materialization',jsonb_build_object('status',case when v_materialized.importacao_id is null
      then 'PENDENTE' else 'MATERIALIZADO' end,'completedAt',v_materialized.completed_at),
    ...
    'lessons',coalesce((select jsonb_agg(jsonb_build_object(
      'id',a.id,'operationalLessonId',(select m.aula_id from internal_academic.diario_aulas_materializadas m where m.aula_historica_id=a.id),
      'sourceKey',a.source_key,'date',a.data_aula,'dateField',a.fonte -> 'date',
    ...
    'students',coalesce((select jsonb_agg(jsonb_build_object(
      ...
      'partial',resultado.media_parcial,
      'final',resultado.media_final,
      'recovery',resultado.nota_rec,
      'gradeState',r.notas_estado,'frequency',resultado.frequencia_percent,'frequencyState',r.frequencia_estado,
