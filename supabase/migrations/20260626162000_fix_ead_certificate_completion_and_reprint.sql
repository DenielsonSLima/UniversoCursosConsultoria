...
with invalid_ead_certificates as (
  select
    ca.id,
    ca.matricula_id,
    ca.codigo_validacao
  from public.certificados_academicos ca
  join public.cursos c on c.id = ca.curso_id
  left join public.ead_aluno_progresso ep
    on ep.aluno_id = ca.aluno_id
   and ep.curso_id = ca.curso_id
  where ca.modalidade = 'EAD'
    and ca.status = 'FINALIZADO'
    and not coalesce(public.ead_progress_meets_completion(ep.progress, c.ead_config), false)
),
revoked_documents as (
  update public.documentos_validacao dv
  set
    status = 'REVOGADO',
    revogado_em = coalesce(dv.revogado_em, now()),
    updated_at = now()
  from invalid_ead_certificates invalid
  where dv.codigo = invalid.codigo_validacao
    and dv.documento = 'certificado_ead'
  returning dv.codigo
),
cancelled_certificates as (
  update public.certificados_academicos ca
  set
    status = 'CANCELADO',
    metadados = coalesce(ca.metadados, '{}'::jsonb) || jsonb_build_object(
      'canceladoAutomaticamenteEm', now(),
      'motivoCancelamento', 'Certificado EAD gerado sem conclusão completa de aulas, atividades, vídeos e prova.'
    ),
    updated_at = now()
  from invalid_ead_certificates invalid
  where ca.id = invalid.id
  returning ca.matricula_id
)
update public.matriculas m
set status = 'ATIVO'
from cancelled_certificates cancelled
where m.id = cancelled.matricula_id
  and upper(coalesce(m.status, '')) = 'CONCLUIDO';

update public.ead_aluno_progresso ep
set progress = ep.progress - 'completedAt' - 'certificateId'
from public.cursos c
where c.id = ep.curso_id
  and c.modalidade = 'EAD'
  and not coalesce(public.ead_progress_meets_completion(ep.progress, c.ead_config), false)
  and (ep.progress ? 'completedAt' or ep.progress ? 'certificateId');

grant execute on function public.ead_progress_meets_completion(jsonb, jsonb) to anon, authenticated;
