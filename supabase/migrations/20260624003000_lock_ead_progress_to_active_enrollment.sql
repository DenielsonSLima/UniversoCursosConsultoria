create index if not exists idx_matriculas_aluno_turma_status
  on public.matriculas (aluno_id, turma_id, status);

create index if not exists idx_turmas_curso_id
  on public.turmas (curso_id);
...
begin
  ...
  if p_action = 'toggle_content' then
    ...
    select ead_config into v_course_config from public.cursos where id = p_curso_id and modalidade = 'EAD';
