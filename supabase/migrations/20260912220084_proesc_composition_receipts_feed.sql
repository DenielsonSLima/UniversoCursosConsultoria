...
CREATE OR REPLACE FUNCTION public.list_financial_receipts_secure(p_company_id uuid DEFAULT NULL::uuid, p_polo_id uuid DEFAULT NULL::uuid, p_payment_start date DEFAULT NULL::date, p_payment_end date DEFAULT NULL::date, p_search text DEFAULT NULL::text, p_origin text DEFAULT 'TODOS'::text, p_environment text DEFAULT 'production'::text, p_page integer DEFAULT 1, p_page_size integer DEFAULT 20)
 RETURNS jsonb
 LANGUAGE plpgsql
  ...
  page_enriched AS (
    SELECT
      ...
      CASE WHEN receipt.origem = 'HISTORICO_MIGRADO'
          AND composition.composicao_status IS DISTINCT FROM 'CONCILIADO_POR_CONFERENCIA_PROESC'
        THEN NULL ELSE composition.juros END AS juros_aplicados,
      CASE WHEN receipt.origem = 'HISTORICO_MIGRADO'
          AND composition.composicao_status IS DISTINCT FROM 'CONCILIADO_POR_CONFERENCIA_PROESC'
        THEN NULL ELSE composition.multa END AS multa_aplicada,
      CASE WHEN receipt.origem = 'HISTORICO_MIGRADO'
          AND composition.composicao_status IS DISTINCT FROM 'CONCILIADO_POR_CONFERENCIA_PROESC'
        THEN NULL ELSE composition.acrescimo END AS acrescimo_aplicado,
      CASE WHEN receipt.origem = 'HISTORICO_MIGRADO'
          AND composition.composicao_status IS DISTINCT FROM 'CONCILIADO_POR_CONFERENCIA_PROESC'
        THEN NULL ELSE composition.desconto END AS desconto_aplicado,
      CASE WHEN receipt.origem = 'HISTORICO_MIGRADO'
          AND composition.composicao_status IS DISTINCT FROM 'CONCILIADO_POR_CONFERENCIA_PROESC'
      ...
      CASE
        WHEN receipt.origem = 'HISTORICO_MIGRADO'
          AND composition.composicao_status IS DISTINCT FROM 'CONCILIADO_POR_CONFERENCIA_PROESC'
          THEN 'HISTORICO_SEM_COMPOSICAO'
      ...
      CASE WHEN composition.composicao_status = 'CONCILIADO_POR_CONFERENCIA_PROESC' THEN 'CONFERENCIA_PROESC'
    ...
    LEFT JOIN LATERAL public.resolve_integrated_receivable_financial_composition(
      ...
      OR EXISTS (SELECT 1 FROM internal_proesc.obligation_links link WHERE link.receivable_id = receipt.id)
  )
