-- Paridade do workspace de Contratos com a Pasta de Identificação: os cartões
-- precisam receber identidade e matrícula canônicas, sem afirmar que CPF/RG
-- estão ausentes quando apenas não faziam parte do payload.
...
begin
  ...
  select coalesce(
    jsonb_agg(
      ...
      ) order by student.nome, class.nome, enrollment.id
    ),
    '[]'::jsonb
  )
  into v_targets
  ...
  select coalesce(
    jsonb_agg(
      jsonb_build_object(
        ...
        'curso_nome', course.nome,
        'modalidade', upper(course.modalidade)
      ) order by course.nome, class.nome, class.id
    ),
    '[]'::jsonb
  )
  into v_turmas
...
comment on function public.get_secretaria_contratos_aluno_workspace_secure(uuid) is
  'Workspace canônico do Contrato do Aluno com identidade e matrícula suficientes para o padrão visual da Secretaria.';
