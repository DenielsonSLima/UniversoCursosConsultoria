-- Endurece as agregacoes financeiras canonicas apos a revisao cruzada:
...
BEGIN
  IF auth.role() IS DISTINCT FROM 'service_role'
     AND (
       (
         p_polo_id IS NULL
         AND public.is_gestor_global() IS NOT TRUE
       )
       OR (
         p_polo_id IS NOT NULL
         AND public.is_gestor_for_polo(p_polo_id) IS NOT TRUE
       )
       OR NOT (
         public.gestor_has_module('caixa') IS TRUE
         OR public.gestor_has_financeiro_tab('transferencias') IS TRUE
       )
     )
  THEN
    RAISE EXCEPTION 'Acesso ao resumo de transferencias fora do escopo autorizado.'
      USING ERRCODE = '42501';
  END IF;
...
-- Mantem o resumo geral e o resumo agrupado sob o mesmo contrato temporal:
-- no escopo mensal, datas manuais restringem o mes em vez de serem ignoradas.
...
BEGIN
  ...
  ELSE
    ...
    v_due_end_exclusive := CASE
      WHEN p_due_end IS NULL THEN NULL
      ELSE p_due_end + 1
    END;
  END IF;
  ...
  SELECT
    ...
    coalesce(sum(despesa.valor) FILTER (
      WHERE despesa.status IN ('PENDENTE', 'VENCIDO')
    ), 0)::numeric(15, 2),
    count(*) FILTER (
      WHERE despesa.status = 'VENCIDO'
         OR (
           despesa.status = 'PENDENTE'
           AND despesa.data_vencimento < CURRENT_DATE
         )
    )
  ...
  WHERE despesa.tipo = p_tipo
    ...
    AND (
      v_due_end_exclusive IS NULL
      OR despesa.data_vencimento < v_due_end_exclusive
    )
    AND (
    ...
    AND (
      v_search IS NULL
      OR despesa.descricao ILIKE '%' || v_search_pattern || '%' ESCAPE E'\\'
      OR categoria.nome ILIKE '%' || v_search_pattern || '%' ESCAPE E'\\'
      OR parceiro.nome ILIKE '%' || v_search_pattern || '%' ESCAPE E'\\'
      OR polo.nome ILIKE '%' || v_search_pattern || '%' ESCAPE E'\\'
    );
END;
$$;
