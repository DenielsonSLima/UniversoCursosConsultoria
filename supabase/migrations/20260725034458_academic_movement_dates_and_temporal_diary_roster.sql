-- A data acadêmica efetiva da saída define a participação do aluno em cada
-- disciplina. O registro permanece imutável; esta migration apenas torna a
-- leitura do diário temporal e valida novas datas informadas.

create index if not exists matricula_movimentacoes_status_date_idx
...
create index if not exists aulas_turma_disciplina_data_idx
  on public.aulas_turma (turma_id, disciplina_id, data_aula);
...
as $function$
  with inicio_disciplina as (
    select min(a.data_aula) as primeira_aula
  ...
  left join lateral (
    select mm.data_movimentacao
    from public.matricula_movimentacoes mm
    where mm.matricula_id = m.id
      and upper(coalesce(mm.status_novo, '')) = upper(coalesce(m.status, ''))
      and upper(coalesce(mm.status_novo, '')) in (
        'CANCELADO',
        'DESISTENTE',
        'TRANSFERIDO'
      )
    order by mm.created_at desc, mm.data_movimentacao desc, mm.id desc
    limit 1
  ) saida on true
  where m.turma_id = p_turma_id
    and (
      upper(coalesce(m.status, '')) not in (
        'CANCELADO',
        'DESISTENTE',
        'TRANSFERIDO'
      )
      ...
      or exists (
        ...
        where ap.matricula_id = m.id
          and ap.disciplina_id = p_disciplina_id
      )
    );
$function$;

revoke all on function internal_academic.p2_diario_matriculas_elegiveis_20260725(
  uuid, uuid
) from public, anon, authenticated;

create or replace function internal_academic.p1_get_diario_alunos_20260719(
  p_turma_id uuid,
  p_disciplina_id uuid
)
returns table(
...
begin
  ...
  join public.matriculas m on m.id = elegivel.matricula_id
  ...
  base as (
    ...
    left join lateral (
      ...
      where f.turma_id = p_turma_id
        ...
        and (
          aluno.data_saida is null
          or a.data_aula is null
          or a.data_aula <= aluno.data_saida
        )
    ) frequencias on true
...
-- Datas de saída não podem ser futuras nem anteriores ao ingresso da matrícula.
...
declare
  ...
  v_status text;
  v_data_movimentacao date := coalesce(
    p_data_movimentacao,
...
begin
  ...
  if v_data_movimentacao > v_hoje then
    raise exception 'A data da movimentação não pode ser futura.';
  end if;
  if v_data_matricula is not null and v_data_movimentacao < v_data_matricula then
...
declare
  ...
  v_data_transferencia date := coalesce(
...
comment on function internal_academic.p2_diario_matriculas_elegiveis_20260725(
  uuid, uuid
) is
  'Relação temporal canônica do diário: saída no dia ou depois da primeira aula preserva o aluno na disciplina; disciplinas posteriores o excluem.';
