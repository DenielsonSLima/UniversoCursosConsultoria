-- Coordenação de curso é uma capacidade derivada de um parceiro Professor.
-- Ela não cria perfil, credencial ou portal separado. O papel COORDENADOR
-- permanece disponível apenas para as RPCs de revisão e assinatura.
    ...
    SELECT
      ...
      coalesce(perfil.scopes, '[]'::jsonb) || coalesce((
        ...
        FROM (
          ...
          FROM coordenacao_scopes
          WHERE "contextId" = perfil."contextId"
        ) AS escopo
      ), '[]'::jsonb) AS scopes,
      perfil."firstAccess"
