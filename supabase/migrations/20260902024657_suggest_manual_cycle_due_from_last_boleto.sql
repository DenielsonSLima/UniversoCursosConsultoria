...
revoke all on function
  internal_academic.technical_manual_cycle_due_from_last_boleto(date)
  from public, anon, authenticated, service_role;

do $manual_cycle_due_self_check$
begin
  if internal_academic.technical_manual_cycle_due_from_last_boleto(
    date '2026-01-15'
  ) is distinct from date '2026-02-15'
    or internal_academic.technical_manual_cycle_due_from_last_boleto(
      date '2026-01-31'
    ) is distinct from date '2026-02-28'
    or internal_academic.technical_manual_cycle_due_from_last_boleto(
      date '2028-01-31'
    ) is distinct from date '2028-02-29'
    or internal_academic.technical_manual_cycle_due_from_last_boleto(
      date '2026-03-31'
    ) is distinct from date '2026-04-30'
  then
    raise exception 'Autoverificação da sugestão de vencimento falhou.';
  end if;
end;
...
begin
  ...
  if v_next_cycle is not null and v_previous_cycle >= 1 then
    if v_last_run.matricula_id is not null
      and v_last_run.cycle_number = v_previous_cycle
    then
      select max(receivable.data_vencimento) into v_last_due
      from public.contas_receber receivable
      where receivable.id = any(v_last_run.receivable_ids)
        and upper(coalesce(receivable.tipo_lancamento, '')) = 'PARCELA'
        and receivable.parcela_numero between 1
          and v_last_run.expected_installment_count;
    else
      select max(receivable.data_vencimento) into v_last_due
      from public.contas_receber receivable
      where receivable.matricula_id = p_matricula_id
  ...
  return jsonb_build_object(
    ...
    'primeiroVencimentoSugerido', pg_catalog.to_char(
      v_suggested_due, 'YYYY-MM-DD'
    ),
