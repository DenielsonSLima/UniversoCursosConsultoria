-- Separa receitas realizadas da carteira recorrente do Caixa.
-- EAD permanece nas entradas recebidas, mas não participa de previsão parcelada.
-- Todos os agrupamentos e ajustes financeiros são calculados no backend.

ALTER FUNCTION public.get_caixa_relatorio_mensal_detalhado_secure(uuid, date)
  RENAME TO get_caixa_relatorio_mensal_detalhado_v2_core;

REVOKE ALL ON FUNCTION public.get_caixa_relatorio_mensal_detalhado_v2_core(uuid, date)
  FROM PUBLIC, anon, authenticated, service_role;

CREATE FUNCTION public.get_caixa_relatorio_carteira_recorrente_core(
  p_polo_id uuid,
  p_inicio date,
  p_fim date
)
    ...
    SELECT
      ...
      CASE
        WHEN upper(cr.status) = 'PAGO'
         AND cr.data_pagamento >= p_inicio
         AND cr.data_pagamento < p_fim
         AND cr.manual_settlement_id IS NOT NULL
         AND cr.manual_settlement_reversed_at IS NULL
...
BEGIN
  ...
  v_relatorio := public.get_caixa_relatorio_mensal_detalhado_v2_core(
    p_polo_id,
    p_competencia
  );
  v_carteira := public.get_caixa_relatorio_carteira_recorrente_core(
    p_polo_id,
    v_inicio,
    v_fim
  );

  RETURN (v_relatorio - 'resumo_turmas') || jsonb_build_object(
    'versao', 3,
    'resumo_cursos', v_carteira -> 'resumo_cursos',
    'analise_recorrente', v_carteira -> 'analise_recorrente'
  );
END;
$$;
...
COMMENT ON FUNCTION public.get_caixa_relatorio_carteira_recorrente_core(uuid, date, date) IS
  'Agrega carteira não EAD por curso, modalidade e turma, incluindo ajustes confirmados.';
COMMENT ON FUNCTION public.get_caixa_relatorio_mensal_detalhado_secure(uuid, date) IS
  'Prestação mensal detalhada do Caixa v3, com carteira recorrente calculada no backend.';
