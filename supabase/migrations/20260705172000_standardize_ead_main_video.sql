create or replace function public.ead_config_required_video_count(p_config jsonb)
returns int
language sql
immutable
as $$
  select case
    when coalesce(p_config->>'videoUrl', p_config->>'videoPrincipalUrl', '') <> '' then 1
    when exists (
      select 1
      from jsonb_array_elements(coalesce(p_config->'conteudos', '[]'::jsonb)) item
      where coalesce(item->>'videoUrl', '') <> ''
    ) then 1
    else 0
  end;
$$;
...
with legacy_video as (
  select c.id, legacy.video_url
  from public.cursos c
  cross join lateral (
    select item->>'videoUrl' as video_url
    from jsonb_array_elements(coalesce(c.ead_config->'conteudos', '[]'::jsonb)) item
    where coalesce(item->>'videoUrl', '') <> ''
    limit 1
  ) legacy
...
set ead_config = jsonb_set(coalesce(c.ead_config, '{}'::jsonb), '{videoUrl}', to_jsonb(legacy_video.video_url), true)
from legacy_video
where c.id = legacy_video.id;

update public.cursos
set ead_config = jsonb_set(coalesce(ead_config, '{}'::jsonb), '{videoUrl}', to_jsonb('https://vimeo.com/1207083868'::text), true)
where modalidade = 'EAD'
  and nome = 'Auxiliar Administrativo'
  and coalesce(ead_config->>'videoUrl', '') = '';
...
set ead_config = jsonb_set(
  ...
  '{conteudos}',
  coalesce((
    select jsonb_agg(item - 'videoUrl' order by ord)
...
declare
  ...
  v_content_total int := jsonb_array_length(v_conteudos);
  v_activity_total int := case
    when coalesce((v_regras->>'exigirAtividades')::boolean, true) then jsonb_array_length(v_atividades)
    else 0
  end;
  v_video_total int := case
    when coalesce((v_regras->>'exigirVideosConcluidos')::boolean, true) then public.ead_config_required_video_count(p_config)
    else 0
  end;
  v_min_score int := coalesce((p_config#>>'{provas,0,notaMinima}')::int, 70);
begin
  return jsonb_array_length(coalesce(p_progress->'completedContentIds', '[]'::jsonb)) >= v_content_total
    and jsonb_array_length(coalesce(p_progress->'completedActivityIds', '[]'::jsonb)) >= v_activity_total
    and jsonb_array_length(coalesce(p_progress->'completedVideoIds', '[]'::jsonb)) >= v_video_total
    and jsonb_array_length(v_questions) >= 10
    and coalesce((p_progress->>'quizScore')::int, -1) >= v_min_score;
end;
$$;
...
begin
  ...
  v_video_total := case
    when coalesce((v_regras->>'exigirVideosConcluidos')::boolean, true) then public.ead_config_required_video_count(v_config)
    else 0
  end;
  ...
  return jsonb_build_object(
    ...
    'summary', jsonb_build_object(
      ...
      'completedAt', case when v_course_completed and v_progress_json ? 'completedAt' then (v_progress_json->>'completedAt')::numeric else null end,
