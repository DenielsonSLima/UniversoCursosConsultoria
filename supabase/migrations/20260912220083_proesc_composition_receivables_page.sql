...
AS $function$
  ...
  filtered AS (
    SELECT
      ...
      CASE
        WHEN composition.composicao_status = 'CONCILIADO_POR_CONFERENCIA_PROESC'
      ...
      CASE WHEN composition.composicao_status = 'CONCILIADO_POR_CONFERENCIA_PROESC'
          THEN ROUND(composition.juros, 2)
        WHEN cr.manual_settlement_interest_cents IS NULL THEN NULL
        ELSE ROUND(cr.manual_settlement_interest_cents::numeric / 100, 2)
      END AS juros_aplicados,
      CASE WHEN composition.composicao_status = 'CONCILIADO_POR_CONFERENCIA_PROESC'
          THEN ROUND(composition.multa, 2)
        WHEN cr.manual_settlement_penalty_cents IS NULL THEN NULL
