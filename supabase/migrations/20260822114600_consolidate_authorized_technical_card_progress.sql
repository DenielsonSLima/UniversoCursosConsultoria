-- Uma única fotografia autorizada alimenta o card técnico. Conclusão e item
-- atual vêm de turmas_disciplinas.concluida; horas não inferem encerramento.
...
BEGIN
  ...
  IF coalesce(auth.role(), '') <> 'service_role'
    AND EXISTS (
      SELECT 1
      FROM unnest(p_turma_ids) requested(requested_id)
      WHERE NOT coalesce(public.can_operate_turma_academics(requested.requested_id), false)
    )
  THEN
  ...
  IF EXISTS (
    ...
    RAISE EXCEPTION 'Uma ou mais turmas solicitadas não existem.' USING ERRCODE = '23514';
  END IF;

  RETURN QUERY
  WITH requested_ids AS (
    SELECT requested_id, request_order
    FROM unnest(p_turma_ids) WITH ORDINALITY requested(requested_id, request_order)
  ), requested_turmas AS (
...
CREATE OR REPLACE FUNCTION public.get_gestao_turmas_completion_counts(
  p_turma_ids uuid[]
)
RETURNS TABLE (
  turma_id uuid,
...
AS $$
  ...
  FROM public.get_gestao_turmas_academic_progress(p_turma_ids) progress;
$$;
...
COMMENT ON FUNCTION public.get_gestao_turmas_completion_counts(uuid[]) IS
  'Wrapper legado restrito ao service_role; novos consumidores usam exclusivamente academic_progress.';
