-- Reduz o egress do Contas a Receber com paginação real, projeção enxuta
-- e agrupamentos carregados sob demanda. As funções são SECURITY INVOKER
-- para preservar integralmente o escopo das políticas RLS existentes.
  ...
  filtered AS (
    ...
    WHERE cr.categoria = 'MENSALIDADE'
      ...
      AND (
        ...
        OR cr.descricao ILIKE '%' || n.search_term || '%'
        OR pa.nome ILIKE '%' || n.search_term || '%'
        OR pa.cpf_cnpj ILIKE '%' || n.search_term || '%'
        OR t.nome ILIKE '%' || n.search_term || '%'
        OR po.nome ILIKE '%' || n.search_term || '%'
        OR po.cnpj ILIKE '%' || n.search_term || '%'
        OR po.cidade ILIKE '%' || n.search_term || '%'
        OR po.estado ILIKE '%' || n.search_term || '%'
      )
  ),
  scoped AS (
    SELECT *
...
CREATE OR REPLACE FUNCTION public.get_receivables_modality_summary_v2(
  p_modality text,
  p_polo_id uuid DEFAULT NULL,
  p_search text DEFAULT NULL,
  p_due_start date DEFAULT NULL,
  ...
  WITH filtered AS (
    ...
    WHERE cr.categoria = 'MENSALIDADE'
      ...
      AND (
        ...
        OR pa.nome ILIKE '%' || BTRIM(p_search) || '%'
        OR pa.cpf_cnpj ILIKE '%' || BTRIM(p_search) || '%'
        OR t.nome ILIKE '%' || BTRIM(p_search) || '%'
...
REVOKE ALL ON FUNCTION public.get_receivables_modality_page(text, uuid, text, date, date, text, text, text, integer, integer) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_receivables_modality_groups_page(text, uuid, text, date, date, text, text, integer, integer) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.get_receivables_modality_summary_v2(text, uuid, text, date, date) FROM PUBLIC, anon;

GRANT EXECUTE ON FUNCTION public.get_receivables_modality_page(text, uuid, text, date, date, text, text, text, integer, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_receivables_modality_groups_page(text, uuid, text, date, date, text, text, integer, integer) TO authenticated, service_role;
GRANT EXECUTE ON FUNCTION public.get_receivables_modality_summary_v2(text, uuid, text, date, date) TO authenticated, service_role;
