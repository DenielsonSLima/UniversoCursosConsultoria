-- Impede cobranças sem valor e lançamentos em disciplinas já aproveitadas.
...
BEGIN
  ...
  IF jsonb_array_length(
    ...
    LOOP
      ...
      v_origem_id := COALESCE(
        NULLIF(v_item->>'id', ''),
        LOWER(v_tipo) || '-' || v_numero
      );
      ...
      v_valor := COALESCE(
        NULLIF(v_item->>'valor', '')::numeric,
        CASE
          WHEN v_tipo = 'REMATRICULA' THEN COALESCE(
  ...
  ELSIF COALESCE(
    ...
    FOR v_numero IN 1..GREATEST(
      COALESCE(v_turma.qtd_parcelas, 0),
      0
    ) LOOP
      ...
      INSERT INTO public.contas_receber (
        ...
        v_turma.polo_id,
        'Mensalidade ' || v_numero || '/' || v_turma.qtd_parcelas
          || ' - ' || v_turma.nome,
        COALESCE(
          v_matricula.valor_parcela_individual,
          v_turma.valor_parcela,
...
COMMENT ON FUNCTION public.gerar_cobranca_matricula(uuid) IS
  'Gera cobrança inicial autorizada somente quando o valor final é positivo.';
COMMENT ON FUNCTION public.gerar_parcelas_matricula(uuid) IS
  'Gera somente parcelas futuras autorizadas com valor positivo.';

CREATE OR REPLACE FUNCTION public.proteger_lancamento_disciplina_aproveitada()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = ''
...
BEGIN
  IF EXISTS (
    ...
    JOIN public.matricula_aproveitamentos ma
      ON ma.matricula_id = m.id
     AND ma.disciplina_id = NEW.disciplina_id
    WHERE m.turma_id = NEW.turma_id
      AND m.aluno_id = NEW.aluno_id
  ) THEN
    RAISE EXCEPTION
      'Disciplina aproveitada: notas e frequência não podem ser alteradas.'
      USING ERRCODE = '23514';
  END IF;
...
REVOKE ALL ON FUNCTION public.proteger_lancamento_disciplina_aproveitada()
  FROM PUBLIC, anon, authenticated;

DROP TRIGGER IF EXISTS proteger_frequencia_disciplina_aproveitada
  ON public.diario_frequencia;
CREATE TRIGGER proteger_frequencia_disciplina_aproveitada
BEFORE INSERT OR UPDATE ON public.diario_frequencia
FOR EACH ROW
EXECUTE FUNCTION public.proteger_lancamento_disciplina_aproveitada();

DROP TRIGGER IF EXISTS proteger_nota_disciplina_aproveitada
  ON public.diario_notas;
CREATE TRIGGER proteger_nota_disciplina_aproveitada
BEFORE INSERT OR UPDATE ON public.diario_notas
FOR EACH ROW
EXECUTE FUNCTION public.proteger_lancamento_disciplina_aproveitada();

-- Remove somente a cobrança zerada criada pelo cenário de homologação.
DELETE FROM public.contas_receber cr
USING public.matriculas m, public.parceiros p, public.turmas t
WHERE cr.matricula_id = m.id
  AND m.aluno_id = p.id
  AND m.turma_id = t.id
  AND lower(p.email) = 'juliana.transferencia@hml.universo.invalid'
  AND t.codigo = 'HML-ENF-2025-COTIDIANO-01'
  AND cr.valor <= 0
  AND cr.status <> 'PAGO'
  AND cr.gateway_payment_id IS NULL
  AND cr.asaas_payment_id IS NULL;
