...
Deno.serve(async (req: Request) => {
  ...
  if (
    isRateLimitExceeded(
      `whatsapp-embedded-signup:${getClientIp(req)}`,
      10,
      60000,
    )
  ) {
    return respondJson({
      error:
        "Muitas tentativas de conexao em curto periodo. Aguarde alguns instantes.",
    }, 429);
  }
  ...
  if (!supabaseUrl || !serviceRoleKey) {
    return respondJson({
      error: "Ambiente Supabase incompleto para Embedded Signup WhatsApp.",
    }, 500);
  }
  ...
  try {
    ...
    if (body.mode !== "coexistence") {
      throw new Error("Modo de Embedded Signup invalido.");
    }
    ...
    if (event !== COEXISTENCE_FINISH_EVENT) {
      throw new Error(
        "A Meta finalizou o fluxo sem evento de coexistencia. Verifique se o App esta habilitado para WhatsApp Business App onboarding.",
      );
    }
    ...
    const statusPayload: PhoneNumberStatus =
      await resolveCoexistencePhoneNumber({
        graphVersion,
        wabaId,
        preferredPhoneNumberId: phoneNumberId,
        accessToken,
      });
    phoneNumberId = trimOrNull(statusPayload.id) || phoneNumberId;
    if (!phoneNumberId) {
      throw new Error(
        "Nao foi possivel identificar o Phone Number ID em coexistencia.",
      );
    }
      ...
      try {
        ...
        warnings.push(
          error instanceof Error
            ? error.message
            : `Nao foi possivel iniciar sincronizacao ${syncType}.`,
        );
      }
    }
    ...
    return respondJson({
      error: error instanceof Error
        ? error.message
        : "Erro inesperado no Embedded Signup WhatsApp.",
    }, 400);
  }
});
