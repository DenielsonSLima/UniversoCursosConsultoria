import { readWorkerSecret, logWorkerSecretRead, type WorkerSecretRuntime } from "../_shared/worker-secret-read.ts";
...
export const handleBaneseReconciliationRequest = async (
  req: Request,
  dependencies: WorkerSecretRuntime<typeof createClient> = {},
) => {
  ...
  const admin = (dependencies.createAdmin ?? createClient)(supabaseUrl, serviceRoleKey, {
    auth: { persistSession: false, autoRefreshToken: false },
  });
  const secret = await (dependencies.readSecret ?? readWorkerSecret)(
    admin, "get_banese_reconciliation_worker_secret", {
      minimumLength: 32,
      logger: (metadata) => logWorkerSecretRead("banese worker secret read", metadata),
    },
  );
  if (!secret.ok) return json({ error: "Configuração indisponível." }, 503);
  const configuredSecret = secret.secret;
  ...
  if (!canLaunchAt(createLaunchPacing(startedAt, prepareStartedAt, 1), prepareStartedAt)) {
    return json({ error: "Tempo de preparação esgotado.", code: "PREPARE_BUDGET_EXHAUSTED" }, 503);
  }
  ...
  const pacing = createLaunchPacing(startedAt, Date.now(), targetTitles, prepareStartedAt);
  let cursor = 0;
