...
export const handlePushNotificationDispatch = async (
  req: Request,
  ...
  const secret = await (dependencies.readSecret ?? readWorkerSecret)(
    admin, "get_push_notification_worker_secret", {
      trimForValidation: true,
      logger: (metadata) => logWorkerSecretRead("push worker secret read", metadata),
    },
  );
  if (!secret.ok) return json({ error: "Executor indisponível." }, 503);
  const expectedSecretValue = secret.secret.trim();
