...
Deno.test('erro lógico, HTML 200, rede, payload excessivo e erro semântico não passam como sucesso', async () => {
  for (const make of [() => Response.json({ success: false, data: [] }),
    () => new Response('<html>SECRET_RESPONSE</html>'),
    () => new Response('x'.repeat(4_000_001))]) {
    const result = await testProescV2Token('synthetic-proesc-secret', undefined, () => Promise.resolve(make()), now);
    assert(!result.ok && !JSON.stringify(result).includes('SECRET_RESPONSE'));
  }
  const network = await testProescV2Token('synthetic-proesc-secret', undefined, () => Promise.reject(new Error('SECRET_RESPONSE')), now);
