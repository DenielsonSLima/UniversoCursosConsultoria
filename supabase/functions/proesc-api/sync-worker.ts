...
export async function runProescSync(
  ...
  try {
    ...
    if (credential.error || !saved?.token || !/^[0-9a-f]{32}$/i.test(saved.token)) {
      throw new SyncTelemetryError('CREDENTIAL_UNAVAILABLE');
    }
    stage = 'FETCH';
    ...
    const readers = Array.from({ length: Math.min(3, tasks.length) }, async () => {
      while (readPosition < tasks.length && !controller.signal.aborted) {
        ...
        try {
          const page = await client.accountingData(period);
          telemetry.http.push({ ...period, httpStatus: 200, durationMs: Math.max(0, Math.min(120000, Date.now() - started)), errorCode: null });
          allRows.push(...page.rows.filter((row) => allowedKeys.has(`${row.identity.unitId}:${row.externalKey}`)));
        } catch (error) {
          const safe = safeSyncError(error, 'FETCH');
          if (safe.code !== 'INVALID_REQUEST') telemetry.http.push({ ...period, httpStatus: safe.httpStatus, durationMs: Math.max(0, Math.min(120000, Date.now() - started)), errorCode: safe.code });
          markSyncFailure(telemetry, error, 'FETCH');
          throw error;
        }
      }
    });
    ...
    if (controller.signal.aborted) throw new SyncTelemetryError('TIMEOUT');
    stage = 'CREDENTIAL';
    ...
    const processLink = async (link: SyncLink) => {
      ...
      try {
        ...
        if (snapshot.error) throw new SyncTelemetryError('SNAPSHOT_REJECTED');
        ...
        if (result.error) {
          if (controller.signal.aborted) throw new SyncTelemetryError('TIMEOUT');
          // Preserve progress after CAS/guard rejection; the source is durable.
          counts.review++;
          telemetry.items.push({ linkId: link.linkId, snapshotId, result: 'REVIEW', stage: itemStage, errorCode: 'APPLY_REJECTED' });
          return;
        }
        const state = (result.data as { result?: string })?.result;
        const outcome = state === 'APPLIED' || state === 'UNCHANGED' ? state : 'REVIEW';
        ...
        telemetry.items.push({ linkId: link.linkId, snapshotId, result: 'FAILED', stage: itemStage,
          errorCode: safeSyncError(error, itemStage).code });
        markSyncFailure(telemetry, error, itemStage);
        throw error;
      }
    };
    ...
    if (controller.signal.aborted && counts.failed === 0) {
      counts.failed++;
      markSyncFailure(telemetry, new SyncTelemetryError('TIMEOUT'), 'RUNTIME');
    }
  } catch (error) {
    counts.failed++;
    markSyncFailure(telemetry, error, stage);
  ...
  await runtime('finish', {
    ...
    success: counts.failed === 0 && completedCount === claim.links.length, counts, telemetry,
  });
