...
export const cancelBaneseReceivableBeforeManualSettlement = async (
  ...
  if (String(receivable.gateway_payment_method || "").toUpperCase() !== "BOLETO") {
    throw new Error("A baixa manual Banese exige um boleto registrado.");
  }
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(String(receivable.id || ""))) {
    throw new Error("Cobranca Banese invalida para baixa manual.");
  }

  const environment: Environment = receivable.gateway_environment === "production"
    ? "production"
    : "sandbox";
  ...
  const metadata = credential?.metadata && typeof credential.metadata === "object"
    ? credential.metadata
    : {};
