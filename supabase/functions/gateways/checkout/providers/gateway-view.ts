...
export const paymentResponseFromReceivable = (
  ...
  return {
    ...
    method: receivable?.gateway_payment_method || context.charge.method,
    installments: Number(
      receivable?.gateway_installments || context.charge.installmentCount,
    ),
    status: receivable?.gateway_status || receivable?.asaas_status || null,
    value: Number(receivable?.valor ?? context.charge.value),
    courseName: context.course.nome,
    recipient: EAD_PAYMENT_RECIPIENT,
    dueDate: receivable?.data_vencimento || context.charge.dueDate,
    invoiceUrl,
    bankSlipUrl,
