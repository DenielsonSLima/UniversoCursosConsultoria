...
import { resolveEnrollmentFinancialTerms } from "./enrollment-financial-terms.ts";
...
export const buildCheckoutCharge = (input: {
  ...
  if (!value || value <= 0) {
    throw new Error(
      "Valor do curso/turma ainda nao configurado para cobranca.",
    );
  }

  const { financeiroConfig } = resolveCourseConfiguredPayment(input.course);
  const turma = input.turma || {};
  const terms = input.applyTurmaAdjustments
    ? resolveEnrollmentFinancialTerms({
      turma,
      matricula: input.matricula,
    })
    : { discountValue: 0, interestPercent: 0, fineValue: 0 };
  const { discountValue, interestPercent, fineValue } = terms;
  const discountEnabled = input.applyTurmaAdjustments &&
    turma?.aplicar_desconto_matricula === true;
  const penaltyEnabled = input.applyTurmaAdjustments &&
    turma?.aplicar_multa_juros_matricula !== false;
  if (discountEnabled && discountValue >= value && discountValue > 0) {
    throw new Error(
      "Desconto de pontualidade da matricula deve ser menor que o valor cobrado.",
    );
  }
  ...
  return {
    ...
    discount: discountApplies
      ? { value: discountValue, dueDateLimitDays: 0, type: "FIXED" }
      : null,
...
export const buildOnlinePaymentPayload = (
  ...
  if (typeof charge.daysAfterDueDateToRegistrationCancellation === "number") {
    payload.daysAfterDueDateToRegistrationCancellation =
      charge.daysAfterDueDateToRegistrationCancellation;
