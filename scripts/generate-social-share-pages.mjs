...
const ROUTES = [
  ...
  publicRoute(
    ...
    { outputFile: 'ead-detail.html', omitUrlMetadata: true },
  ),
  publicRoute(
    '/cursos-livres',
    'cursos-livres',
  ...
  publicRoute(
    ...
    { outputFile: 'especializacao-detail.html', omitUrlMetadata: true },
  ),
  publicRoute(
    '/ensino-superior',
    'ensino-superior',
...
const buildRouteHtml = (sourceHtml, metadata) => {
  ...
  const imageUrl = `${SITE_URL}/social-share/${metadata.slug}.jpg`;
...
for (const slug of new Set(ROUTES.map(({ slug }) => slug))) {
  await assertImageExists(slug);
}
