...
- **Supabase somente via MCP, CLI proibida:** Neste projeto, qualquer ação que envolva Supabase deve usar MCP Supabase (`execute_sql`, `apply_migration`, logs, Edge Functions, Storage, Auth, RLS ou ferramenta equivalente disponível). É proibido executar qualquer comando do binário `supabase` (`supabase migration list`, `supabase db push`, `supabase functions deploy`, `supabase link`, `supabase status`, `supabase start`, etc.), mesmo para consulta/listagem. Já houve falhas `401 Unauthorized` na CLI enquanto o MCP estava autenticado e autorizado; erro da CLI não é bloqueio e não deve ser tentado quando MCP estiver disponível.
...
- **Supabase CLI proibida:** Não executar nem sugerir nenhum comando `supabase ...` neste projeto, incluindo comandos aparentemente inofensivos como `supabase migration list`, `supabase status` ou `supabase start`. Para banco, migrations, logs, Auth, Storage, RLS e Edge Functions, use somente MCP Supabase.
