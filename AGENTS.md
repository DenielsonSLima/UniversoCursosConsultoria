...
## Regra Obrigatória De PDFs Oficiais

- Contratos, carteirinhas, calendários e demais PDFs oficiais devem ter uma única fonte canônica: a prévia, o download e a impressão usam o mesmo Blob PDF gerado a partir do payload preparado pelo backend/RPC.
- Nunca rasterizar a página inteira do documento com `html2canvas`, `canvas`, captura de DOM ou `addImage` para gerar o PDF. Texto, linhas, tabelas, marcações e fundo vetorial devem permanecer nativos e selecionáveis.
- Marca-d’água institucional deve ser desenhada como camada de fundo separada — recurso de imagem isolado ou elementos vetoriais — antes do conteúdo; QR Code, foto e assinatura podem ser recursos isolados, mas jamais uma imagem A4 que substitua o documento.
- O backend/RPC permanece responsável por elegibilidade, dados, ordenação, validade, QR e paginação canônica. O frontend apenas pede o payload e compõe a apresentação vetorial; se o payload não couber, deve falhar de modo explícito em vez de inventar paginação, cálculos ou conteúdo.
- Todo lote que altere emissão de documento deve testar a extração de texto e inspecionar os recursos embutidos, comprovando que não há imagem de página inteira e que a qualidade de impressão corresponde à prévia.
